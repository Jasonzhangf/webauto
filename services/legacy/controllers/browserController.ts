...
export async function navigate(req, res) {
  ...
  try {
    let page = await getSafePage(sessionId);
    ...
    try {
      ...
      // Fallback: open a fresh page and retry once (handles accidental page close or blocked initial page)
      try {
        const s = await getSession(sessionId);
        if (!s?.context) throw e1;
        const fresh = await s.context.newPage();
        await fresh.goto(url, { waitUntil, timeout: timeoutMs });
        s.page = fresh; await saveSession(sessionId, s);
        return res.json({ success: true, url: fresh.url(), note: 'fallback:newPage' });
      } catch (e2) {
        return res.status(500).json({ success: false, error: (e2?.message || e1?.message || 'navigate failed') });
      }
    }
  } catch (e) {
