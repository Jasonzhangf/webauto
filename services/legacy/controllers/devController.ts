...
export async function installPicker(req, res) {
  ...
  try {
    const page = await getPage(sessionId);
    const target = resolveFrame(page, frame);
    // High-priority guard: if captcha (风控) present, abort injection
    try {
      const chk = await target.evaluate(() => {
        try {
          ...
          const el = document.querySelector('.nc-lang-cnt') || document.querySelector('[data-nc-lang]') || document.querySelector('.nc-container') || document.querySelector('[id*="nocaptcha"], [class*="nocaptcha"]');
          return !!(el && vis(el));
        } catch { return false; }
      });
      if (chk) return res.status(412).json({ success:false, error:'captcha present; abort picker install' });
    } catch {}
    ...
    const value = await target.evaluate((src)=>{ try{ var fn=new Function(src); return { ok:true, val: fn() }; }catch(e){ return { ok:false, err:String(e) }; } }, code);
    // persist across navigations: addInitScript on top page
    try { await page.addInitScript(code); } catch {}
    if (!value.ok) return res.status(500).json({ success:false, error:value.err });
    return res.json({ success:true, value:value.val });
  } catch (e) { return res.status(500).json({ success:false, error:e.message }); }
}
