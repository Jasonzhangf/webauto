...
export interface CollectLinksInput {
  ...
  stateClient?: any; // optional StateClient for progress/events
}
...
export async function execute(input: CollectLinksInput): Promise<CollectLinksOutput> {
  ...
  if (input.stateClient) {
    await input.stateClient.pushEvent('phase2:collect:start', {
      keyword: input.keyword,
      targetCount: input.targetCount,
      alreadyCollected: input.alreadyCollectedNoteIds?.length || 0,
    }).catch(() => {});
  }
  const {
    keyword,
 ...
 console.log(`[Phase2CollectLinks] 目标: ${targetCount} 条链接`);
  ...
  while (links.length < targetCount && attempts < maxAttempts) {
    ...
    if (input.stateClient && links.length % 5 === 0) {
      await input.stateClient.updateProgress(links.length, targetCount).catch(() => {});
    }
  ...
  return {
    ...
    // Push individual link event
    if (input.stateClient) {
      await input.stateClient.pushEvent('phase2:link:collected', {
        noteId,
  ...
  if (input.stateClient) {
    await input.stateClient.updateProgress(links.length, targetCount).catch(() => {});
    await input.stateClient.pushEvent('phase2:collect:done', {
