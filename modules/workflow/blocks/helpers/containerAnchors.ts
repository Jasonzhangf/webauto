...
function loadContainerIndex(): Record<string, { website: string; container_file: string }> | null {
  try {
...
function findSiteKeyForContainer(containerId: string, index: Record<string, { website: string; container_file: string }> | null): string | null {
  if (!index) return null;
  const keys = Object.keys(index);
  for (const key of keys) {
    if (containerId.startsWith(`${key}_`)) return key;
  }
  return null;
}
...
async function findContainerDefinition(containerId: string): Promise<ContainerDefinition | null> {
...
function pickPrimarySelector(container: ContainerDefinition): string | null {
