...
async function waitForDetailVisible(profileId, timeoutMs = 5000, intervalMs = 200) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const visible = await isDetailVisible(profileId).catch(() => null);
    if (visible?.detailVisible) {
      return { ok: true, elapsed: Date.now() - start, selector: visible.selector || null };
    }
    await sleep(Math.max(80, intervalMs));
  }
  return { ok: false, elapsed: Date.now() - start };
}

async function waitForDetailClosed(profileId, timeoutMs = 5000, intervalMs = 200) {
  const start = Date.now();
  let lastVisible = await isDetailVisible(profileId).catch(() => null);
  while (Date.now() - start < timeoutMs) {
    if (!lastVisible?.detailVisible) {
      return { ok: true, elapsed: Date.now() - start, visible: lastVisible };
    }
    await sleep(Math.max(80, intervalMs));
    lastVisible = await isDetailVisible(profileId).catch(() => null);
  }
  return { ok: false, elapsed: Date.now() - start, visible: lastVisible };
}
...
async function waitForDetailClose(profileId, attempts = 6, minMs = 220, maxMs = 520) {
  const timeoutMs = Math.max(800, attempts * Math.max(maxMs, minMs, 200));
  const intervalMs = Math.max(120, Math.min(maxMs, minMs));
  const result = await waitForDetailClosed(profileId, timeoutMs, intervalMs);
  return {
    closed: result.ok,
...
async function cleanupDetailContextOnDone({
  ...
  if (!isDirectDetailUrl(afterBackUrl) && !afterBackVisible?.detailVisible) {
    return { attempted: true, cleaned: true, method: 'page_back', beforeUrl: currentUrl || null, afterUrl: afterBackUrl || null, backState };
  }
  ...
  const gotoState = await waitForDetailClose(profileId, 6, 220, 520);
  ...
  if (!isDirectDetailUrl(afterGotoUrl) && !afterGotoVisible?.detailVisible) {
    return { attempted: true, cleaned: true, method: 'goto_list', beforeUrl: currentUrl || null, afterUrl: afterGotoUrl || null, gotoState };
  }
...
export async function executeCloseDetailOperation({ profileId, params = {}, context = {} }) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    if (retryPolicy === 'esc') {
      ...
      await waitForDetailClose(profileId, 2, 200, 400);
