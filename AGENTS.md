...
- 遇到任何执行失败，必须先看日志再做判断；并在反馈中标明日志路径（`~/.webauto/logs` 或 `~/.webauto/download/.../run.log`）。
...
- 代码逻辑禁止使用 fallback 作为默认方案；必须精确处理每个分支，fallback 仅在用户显式要求时允许使用。�?
...
当 `webauto` 命令不可用时，统一用脚本入口执行 UI CLI：

```bash
node bin/webauto.mjs ui cli --help
...
```bash
npm run test:webauto:ui-cli:unit
```

推荐统一入口：

```bash
webauto test --layer all --output ./.tmp/ui-test-report.json
```

层级映射：
L0: `npm run test:e2e-ui:l0`
L1: `npm run test:e2e-ui:l1`
L2: `npm run test:e2e-ui:l2`
L3: `npm run test:e2e-ui:l3`

Test CLI 输出：
终端摘要 + JSON 报告（默认写入 `./.tmp/ui-test-report-<timestamp>.json`）
�?
...
- **三层强制**：公用函数库（`lib/utils`类） → 最小功能模块（单一职责能力） → App/Orchestrator 编排。
- **UI 与 App 强解耦**：UI 只做展示与触发；App 只做编排与生命周期管理；禁止互相内嵌逻辑。
- **唯一真源**：同一能力只允许一个实现入口，禁止重复实现或分叉版逻辑。
- **模块最小化**：功能拆分到最小可复用单元；禁止巨型文件或“万能模块”。
...
- **状态判断必须基于容器锚点**：禁止以 URL 作为页面状态判断标准（例如 `search_result`/`explore`），必须使用对应容器/锚点可见性判定。

- 禁止构�?URL 直达搜索；必须“页面内输入框搜�?+ 回车”�?
- 详情允许直达 **仅限** 已采集的带 `xsec_token` 的安全链接（safe-detail-urls）；无 `xsec_token` 禁止直达
- 禁止读取 `href` 后自行拼接详情链接或 `xsec_token`（�?404/风控）�?
- 详情链接必须通过“点击进入帖子后读取当前 URL（含 xsec_token）”并落盘为 safe-detail-urls
...
- 滚动只允许 PageUp/PageDown（键盘翻页）；禁用 mouse wheel；禁止 scrollBy/scrollTo 等 JS 滚动
...
- 禁止一�?JS 调用的操作（包括 click/scroll/输入/导航/打开标签等）。JS 仅允许用于读取状态（rect/可见�?文本/属性）；任�?JS 操作必须改为协议级操作（mouse/keyboard/container）�?
### 4.1 统一流程基线（强制）
...
- `docs/arch/state-machines/xhs-detail-comments-likes.v2026-03-13.md`（当前评论+点赞环节状态机唯一真源）

## 9. 执行模式约束（强制）

### 9.0 开发/生产分支策略（强制）
- 开发阶段必须使用 `--env debug`：发生错误立即停止（不兜底），便于现场检查与复现。
- 生产阶段使用 `--env prod`：发生错误优先回退到最近 checkpoint 并继续后续任务，尽量完成整体任务。


### 9.1 Daemon 模式（强制）
- **Զ�̻�������ʹ�� daemon����������ֱ������ webauto����ʹ�� relay��**
- 远程必须使用 daemon/relay；本地直接运行 webauto（禁止 relay）。
- 目的：避免会话中断导致任务终止，保证长任务（采集、互动）持续运行
- **macOS LaunchAgent 约束**：LaunchAgent 必须使用 webauto.mjs --daemon run 常驻模式；默认将 StandardOutPath/StandardErrorPath 指向 /dev/null，避免高频输出导�?CPU/IO 异常。需要排查时再临时切回日志文件�?
...
### 9.1.1 标准构建与测试流程（强制�?
- **��Զ��/Session 0 ���ã����ؿ�ֱ������ webauto**
- **远端构建（允许在 Session 0 执行�?*�?
  - `git reset --hard && git clean -fd && git pull --rebase`
  - `npm --prefix apps/desktop-console run build`
  - `npm run build:services`
- 远程/Session 0 运行与测试必须通过 daemon relay；本地直接运行 webauto。
...
- 远程/Session 0 执行脚本必须使用 daemon/relay；本地直接运行 webauto。
...
### 14.1 环节隔离原则
- 当任务在调试某个环节（例如 detail 评论/点赞）时，**只允许修改当前环节**。
- 禁止触碰前置环节（例如 collect）除非用户明确批准。
- 当前环节必须定义唯一状态机文档路径，并在 AGENTS.md 保持可定位。

### 14.2 当前环节状态机唯一真源（评论+点赞）
- 文档路径：`docs/arch/state-machines/xhs-detail-comments-likes.v2026-03-13.md`
- 若执行中发现状态机不清晰：先检索 `MEMORY.md` 与 memsearch，再补充状态机文档，不允许口头约定。

### 14.3 当前环节 debug 固定流程
1. 明确测试目标（本次要验证哪个状态迁移/哪个锚点）。
2. 先对照状态机梳理当前环节全局流转。
3. 每次测试后必须判断：
   - 是**锚点漏洞**（优先修）；
   - 还是**状态机设计问题**（次优先，需新版本设计）。
4. 默认判断：状态机大方向通常可用，优先修复锚点与触发条件细节。
5. 若必须调整状态机：
   - 先写 memory 记录变更原因与证据；
   - 新增新版状态机图（不可覆盖旧图）；
   - 按新图 review 代码并修复；
...
## Clock 巡检规则（新增）
- 所有压力测试必须设置 clock.md 作为巡检锚点
- clock.md 必须包含：背景、当前阻塞点、下次提醒要做的第一步、不能忘的检查项
- 巡检间隔默认 5 分钟，直到任务结束
