...
在你声明的完成任务的内容中，是否都有验证过？验证过的话请提供证据，没有验证的话请验证后再提交结论。

如果你完成了新功能的开发，你确保你的功能是整个项目唯一的该功能实现，并且你有单元测试和回归测试，并且要有 90% 代码覆盖率。

如果你现在需要解决问题，你要确保你修复的位置是唯一的最佳修复点，不是局部修复。

我们的代码需要是模块化的基础功能抽象的模块基础上的编排，尽量把模块的功能做到单一唯一不重复实现。

如果你完成了所有的任务，请记住清理临时文件，脚本以及不在目标文件夹的散落文件。

如果你正在执行开发任务，那就继续执行，在没有达到目标前，按照 task.md 进行推进，遇到问题需要更新 task.md 就更新进度表可以被追踪。
