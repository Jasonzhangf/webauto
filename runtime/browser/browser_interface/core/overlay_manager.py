...
class OverlayManager(IOverlayManager):
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        default_overlay_dir = Path(__file__).resolve().parent.parent / 'overlay_assets'
        custom_overlay = config.get('overlay_dir')
        self.overlay_dir = Path(custom_overlay).expanduser() if custom_overlay else default_overlay_dir
        self.overlay_dir = self.overlay_dir.resolve()
