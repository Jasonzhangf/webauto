...
export class PythonCliBridge {
  ...
  constructor(config: WebAutoConfig) {
    ...
    this.cliPath = path.join(repoRoot, 'runtime/browser/cli/main.py');
  ...
  private async runPythonProcess(args: string[]): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const pythonEnv = Object.assign({}, process.env, {
        PYTHONPATH: this.buildPythonPathEnv()
      });
  ...
  private buildPythonPathEnv(): string {
    const segments = [
      'runtime',
      'runtime/browser',
      'runtime/containers',
      ...
      'runtime/vision',
      'runtime/infra'
    ].map((relPath) => path.join(this.repoRoot, relPath));
    const joined = segments.join(path.delimiter);
    const existing = process.env.PYTHONPATH;
    return existing ? `${joined}${path.delimiter}${existing}` : joined;
  }
}
