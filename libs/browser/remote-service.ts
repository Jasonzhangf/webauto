...
interface BrowserServiceModule {
  startBrowserService: (options: BrowserServiceOptions) => Promise<any>;
}

async function loadService(): Promise<BrowserServiceModule> {
  const distEntry = browserServiceDistEntry();
  ensureBrowserServiceBuild('browser-service');
  const mod = await import(pathToFileURL(distEntry).href);
  return mod as BrowserServiceModule;
}

export async function startBrowserService(options: BrowserServiceOptions = {}): Promise<any> {
