...
export class PostLinkValidationOperation extends BaseAtomicOperation {
   ...
   * 从页面提取链接
   */
  private async extractLinksFromPage(page: any): Promise<string[]> {
   ...
   * 采样链接
   */
  private sampleLinks(links: string[], rate: number): string[] {
   ...
   * 验证链接
   */
  private async validateLinks(links: string[], rules: any[], options: any) {
   ...
   * 应用验证规则
   */
  private async applyValidationRule(link: string, rule: any): Promise<any> {
    const startTime = Date.now();
   ...
   * 验证链接格式
   */
  private validateFormat(link: string, rule: any): any {
    const startTime = Date.now();
   ...
   * 验证链接可访问性
   */
  private async validateAccessibility(link: string, rule: any): Promise<any> {
    const startTime = Date.now();
   ...
   * 验证链接唯一性
   */
  private validateUniqueness(link: string, rule: any): any {
    const startTime = Date.now();
   ...
   * 生成链接hash
   */
  private generateLinkHash(link: string): string {
    let hash = 0;
   ...
   * 验证链接内容
   */
  private async validateContent(link: string, rule: any): Promise<any> {
    const startTime = Date.now();
   ...
   * 估计总有效帖子数
   */
  private estimateTotalValidPosts(totalLinks: number): number {
    if (this.validationStats.totalValidations === 0) return 0;
