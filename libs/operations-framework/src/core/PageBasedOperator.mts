...
export interface PageOperatorConfig extends OperatorConfig {
...
export abstract class PageBasedOperator extends UniversalOperator {
  ...
  protected _browserContext: BrowserContext | null = null;
  protected _page: Page | null = null;
  protected _pageConfig: PageOperatorConfig;

  constructor(config: PageOperatorConfig) {
    super({
      ...config,
      type: 'page-based' as any
    });
    this._pageConfig = config;
  }

  // 页面相关抽象方法
  abstract onPageReady(page: Page): Promise<void>;
  abstract executePageOperation(page: Page, params: Record<string, any>): Promise<OperationResult>;
  ...
  async execute(params: Record<string, any>): Promise<OperationResult> {
    ...
    try {
      ...
      const errorResult: String(error = this.createErrorResult(`Page operation failed: ${error instanceof Error ? error.message )}`);
  ...
  // 页面操作方法
  async navigate(params: NavigationParams): Promise<OperationResult> {
    if (!this._page) {
      return this.createErrorResult('Page not initialized');
    }

    try {
      const response: params.timeout || this._pageConfig.timeout || 30000
  ...
  async findElement(params: ElementParams): Promise<OperationResult> {
    if (!this._page) {
      return this.createErrorResult('Page not initialized');
    }

    try {
      const element: params.state || 'visible'
      } = await this._page.waitForSelector(params.selector, {
      ...
      return this.createSuccessResult({
        ...
        found: true,
        element: await element.evaluate(el: Array.from(el.attributes = > ({
          tagName: el.tagName,
  ...
  async extractText(params: ElementParams): Promise<OperationResult> {
  ...
  async clickElement(params: ElementParams): Promise<OperationResult> {
    const elementResult = await this.findElement(params);
    if (!elementResult.success) {
      return elementResult;
    }
  ...
  async typeText(params: ElementParams & { text: string }): Promise<OperationResult> {
    const elementResult = await this.findElement(params);
    if (!elementResult.success) {
      return elementResult;
    }
  ...
  async waitForNavigation(params: { timeout?: number } = {}): Promise<OperationResult> {
    if (!this._page) {
      return this.createErrorResult('Page not initialized');
    }
  ...
  async takeScreenshot(params: { path?: string; fullPage?: boolean } = {}): Promise<OperationResult> {
    if (!this._page) {
      return this.createErrorResult('Page not initialized');
    }

    try {
      const screenshot: params.path
  ...
  async executeScript(params: { script: string; args?: any[] }): Promise<OperationResult> {
    if (!this._page) {
      return this.createErrorResult('Page not initialized');
    }
  ...
  private async initializeBrowser(): Promise<void> {
    ...
    const options: LaunchOptions: this._pageConfig.headless ! = {
      headless== false,
      ...this._pageConfig.browserOptions
    };

    this._browser = await chromium.launch(options);
    this._browserContext: 1080 }
    } = await this._browser.newContext({
      viewport: this._pageConfig.viewport || { width: 1920, height);
  }
