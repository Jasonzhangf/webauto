...
export class CookieOperator extends NonPageOperator {
  private _cookieStore: Map<string, CookieData>;
  ...
  async executeNonPageOperation(params: CookieParams): Promise<OperationResult> {
  ...
  validateParams(params: CookieParams): boolean {
  ...
  private clearCookies(domain?: string): Promise<OperationResult> {
    try {
      let clearedCount = 0;
  ...
  private listCookies(domain?: string): Promise<OperationResult> {
    try {
      let cookies: CookieData[];

      if (domain) {
        // 列出指定域名的Cookie
        cookies = Array.from(this._cookieStore.values()).filter(
  ...
  // 批量操作
  async importCookies(cookies: CookieData[]): Promise<OperationResult> {
    try {
      let importCount = 0;
  ...
  async exportCookies(domain?: string): Promise<OperationResult> {
    try {
      let cookies: CookieData[];
