...
export class EventDrivenPaginationContainer extends EventDrivenContainer {
  ...
  private paginationStartTime: number = 0;
   ...
   * 开始分页
   */
  async startPagination(): Promise<void> {
   ...
   * 停止分页
   */
  stopPagination(reason: string): void {
    if (!this.isPaginating) return;
   ...
   * 按钮分页模式
   */
  private async startButtonPagination(): Promise<void> {
   ...
   * URL分页模式
   */
  private async startUrlPagination(): Promise<void> {
    const paginationLoop = async () => {
      if (!this.isPaginating) return;
   ...
   * 加载更多分页模式
   */
  private async startLoadMorePagination(): Promise<void> {
    const paginationLoop = async () => {
      if (!this.isPaginating) return;
   ...
   * 无限滚动分页模式
   */
  private async startInfinitePagination(): Promise<void> {
    const paginationLoop = async () => {
      if (!this.isPaginating) return;
   ...
   * 导航到下一个URL
   */
  private async navigateToNextUrl(): Promise<boolean> {
    if (!this.config.urlPattern) return false;
   ...
   * 触发无限滚动
   */
  private async triggerInfiniteScroll(): Promise<boolean> {
    try {
      // 滚动到页面底部
   ...
   * 检查是否到达最后一页
   */
  private async hasReachedLastPage(): Promise<boolean> {
    if (!this.sharedSpace?.page) return false;
   ...
   * 处理页面导航
   */
  private async handlePageNavigation(): Promise<void> {
  ...
  private async getPageSize(): Promise<number> {
    if (!this.sharedSpace?.page) return 0;
   ...
   * 检查是否有新内容
   */
  private hasNewContent(currentPage: PageData): boolean {
    if (this.pageHistory.length < 2) return true;
   ...
   * 设置分页事件处理器
   */
  private setupPaginationEventHandlers(): void {
    // 监听链接目标达到事件
   ...
   * 处理链接目标达到
   */
  private handleLinkTargetReached(): void {
   ...
   * 导出页面历史
   */
  exportPageHistory(format: 'json' | 'csv' = 'json'): string {
    if (format === 'json') {
