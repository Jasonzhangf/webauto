import { MSG_CONTAINER_CHILD_DISCOVERED, MSG_CONTAINER_OPERATION_COMPLETE, MSG_CONTAINER_OPERATION_FAILED, MSG_CONTAINER_ROOT_ALL_OPERATIONS_COMPLETE } from './MessageConstants.js';
/**
 * 容器处理状态
 */
...
export class ContainerStatusTracker {
    ...
    setupListeners() {
        // 监听发现消息
        this.messageBus.subscribe(MSG_CONTAINER_CHILD_DISCOVERED, (msg) => {
            ...
            if (containerId && !this.containerStates.has(containerId)) {
                ...
                console.log('[StatusTracker] Discovered:', containerId, 'Total:', this.totalDiscovered);
            }
        });
        // 监听操作完成消息
        this.messageBus.subscribe(MSG_CONTAINER_OPERATION_COMPLETE, (msg) => {
            const { containerId } = msg.payload;
        ...
        // 监听操作失败消息
        this.messageBus.subscribe(MSG_CONTAINER_OPERATION_FAILED, (msg) => {
            const { containerId } = msg.payload;
            const state = this.containerStates.get(containerId);
            if (state) {
     ...
     * 标记容器处理完成
     */
    markContainerCompleted(containerId) {
        const state = this.containerStates.get(containerId);
        if (state && state.status !== ContainerStatus.COMPLETED && state.status !== ContainerStatus.FAILED) {
            ...
            this.totalCompleted++;
            console.log('[StatusTracker] Completed:', containerId, 'Progress:', this.totalCompleted, '/', this.totalDiscovered);
            this.checkAllComplete();
        }
    }
    checkAllComplete() {
