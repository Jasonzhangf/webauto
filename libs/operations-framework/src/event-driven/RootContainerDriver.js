...
import { ContainerDiscoveryEngine } from './ContainerDiscoveryEngine.js';
import { ContainerOperationExecutor } from './ContainerOperationExecutor.js';
import { ContainerStatusTracker } from './ContainerStatusTracker.js';
import { MSG_CONTAINER_ROOT_SCROLL_START, MSG_CONTAINER_ROOT_SCROLL_COMPLETE, MSG_CONTAINER_ROOT_ALL_OPERATIONS_COMPLETE, MSG_CONTAINER_ROOT_DISCOVER_COMPLETE, MSG_SYSTEM_ERROR, CMD_BROWSER_PAGE_SCROLL, RES_BROWSER_PAGE_SCROLL } from './MessageConstants.js';
/**
 * 根容器驱动器
 * 负责协调发现、执行、滚动循环
 */
export class RootContainerDriver {
    ...
    setupListeners() {
        // 监听发现完成
        this.messageBus.subscribe(MSG_CONTAINER_ROOT_DISCOVER_COMPLETE, async (msg) => {
            // 可以在这里触发操作执行，或者在 runDiscoveryCycle 中直接调用
        });
        // 监听所有操作完成 -> 触发滚动或结束
        this.messageBus.subscribe(MSG_CONTAINER_ROOT_ALL_OPERATIONS_COMPLETE, async (msg) => {
            if (!this.isRunning)
                return;
            const { completed, failed } = msg.payload;
            const totalProcessed = this.variableManager.getRootVariable('totalProcessed') || 0;
    ...
    shouldScroll() {
        if (!this.config.scrollConfig?.enabled)
            return false;
        if (this.scrollCount >= (this.config.scrollConfig.maxScrolls || 10))
            return false;
        // 还可以检查是否到底部等条件
        return true;
    }
