...
async function jsClick(selector, index = 0) {
  log('CLICK', `JS click on ${selector}[${index}]`);
...
async function clickNoteWithRetry(index) {
  ...
  for (let attempt = 0; attempt < MAX_CLICK_RETRY; attempt++) {
    ...
    // 使用JS模拟点击图片
    const clicked = await jsClick('.note-item img', index);
    
    if (!clicked) {
      log('ERROR', `Failed to click image for note ${index}`);
      return { clicked: false };
    }
    
    log('CLICK', `JS click executed on note ${index}`);
...
async function main() {
  log('INIT', 'Starting native click collector v8 (JS click fallback)');
