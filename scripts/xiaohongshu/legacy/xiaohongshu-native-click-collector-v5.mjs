...
// 获取元素的绝对坐标（pageXY）
async function getElementAbsoluteCoords(selector, index = 0) {
...
// 原生鼠标点击（使用绝对坐标）
async function nativeClickAtPageCoords(x, y) {
  log('CLICK', `Native page click at (${Math.round(x)}, ${Math.round(y)})`);
...
async function clickNoteWithRetry(index) {
  ...
  for (let attempt = 0; attempt < MAX_CLICK_RETRY; attempt++) {
    ...
    // 获取元素绝对坐标
    const coords = await getElementAbsoluteCoords('.note-item', index);
    ...
    log('COORD', `Element: ${Math.round(coords.width)}x${Math.round(coords.height)}`);
    ...
    // 原生点击（使用页面绝对坐标）
    await nativeClickAtPageCoords(coords.x, coords.y);
...
async function main() {
  log('INIT', 'Starting native click collector v5 (absolute coords + retry)');
