    ...
    payload: {
      ...
      script: `(() => {
          ...
          return noteItems.map((el, index) => {
            ...
            const noteId = (() => {
              const match = link.match(/\/explore\/([a-f0-9]+)/);
              return match ? match[1] : '';
            })();
            return {
              index,
...
async function main() {
  ...
  const uniqueNotes = new Map(); // 用Map来去重，key为title+author
  let noNewCount = 0;
  ...
  while (collected.length < TARGET_COUNT && noNewCount < MAX_NO_NEW) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      // 检查是否已经采集过相同的笔记（基于标题+作者）
      const uniqueKey = `${item.text.substring(0, 50)}-${item.index}`;
