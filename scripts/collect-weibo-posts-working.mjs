    ...
    const extractScript = `
      (function() {
        ...
        posts.forEach((post, index) => {
          ...
          // URL - 查找微博状态链接
          const link = post.querySelector('a[href*="weibo.com"][href*="status"]') || post.querySelector('a[href*="weibo.com/u/"]');
          ...
          // 只收集有 URL 或内容的帖子
          if ((data.url || data.content) && data.content) {
            results.push(data);
          }
        });
    ...
    if (Array.isArray(posts)) {
      ...
      posts.forEach(post => {
        if (post.url && !collectedPosts.has(post.url)) {
          ...
          newPosts++;
        } else if (!post.url && post.content) {
          // 如果没有URL，使用内容作为唯一标识
          const contentKey = post.content.substring(0, 100);
          if (!collectedPosts.has(contentKey)) {
            collectedPosts.set(contentKey, post);
            newPosts++;
          }
        }
      });
