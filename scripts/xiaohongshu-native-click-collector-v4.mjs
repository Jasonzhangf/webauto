...
// 原生鼠标点击（使用 move + down + up）
async function nativeClick(x, y) {
  log('CLICK', `Native click at (${Math.round(x)}, ${Math.round(y)})`);
...
async function clickNoteImageNative(index) {
  ...
  log('COORD', `Element: ${Math.round(coords.width)}x${Math.round(coords.height)} at (${Math.round(coords.x)}, ${Math.round(coords.y)})`);
...
async function main() {
  log('INIT', 'Starting native click collector v4 (enhanced validation)');
  ...
  while (collected.length < TARGET_COUNT && noNewCount < MAX_NO_NEW) {
    ...
    for (let i = 0; i < newItems.length; i++) {
      ...
      log('URL', `Current URL: ${detailUrl.substring(0, 80)}...`);
