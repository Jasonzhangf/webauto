...
class EventDrivenCollector {
  ...
  async handleExpandButtonAppear(payload) {
    ...
    try {
      ...
      // Wait a bit for content to load, then trigger another container match
      await new Promise(r => setTimeout(r, 1000));
      
      // Trigger container match to find new containers after expand
  ...
  async extractPost(container) {
    ...
    if (!result.success || !result.data?.extracted || !result.data.extracted.length) {
      log('DEBUG', `No extraction result for ${container.containerId}`);
      return null;
    }
  ...
  async scrollPage() {
    ...
    // Trigger container match after scroll to find new content
    await new Promise(r => setTimeout(r, 1000));
    await post('/v1/controller/action', {
      action: 'containers:match',
      payload: {
  ...
  async generateMarkdown() {
    ...
    const filename = 'weibo_posts_150_event_driven_v2.md';
