...
class FixedExtractor {
  ...
  async executeExtraction() {
    const result = await post('/v1/controller/action', {
      ...
      payload: {
        profile: PROFILE,
        script: '(function() { const articles = Array.from(document.querySelectorAll("article[class*=\\"Feed_wrap_\\"]")); const posts = []; for (const article of articles.slice(0, 10)) { const links = Array.from(article.querySelectorAll("a[href*=\\"weibo.com\\"]")); let postUrl = ""; for (const link of links) { const href = link.href; if (href.includes("/status/") || href.includes("/detail/")) { postUrl = href; break; } } const header = article.querySelector("header"); const authorLink = header ? header.querySelector("a[href*=\\"weibo.com\\"]") : null; const author = authorLink ? authorLink.textContent.trim() : ""; const contentEl = article.querySelector("div[class*=\\"detail_wbtext\\"]"); const content = contentEl ? contentEl.textContent.trim() : ""; const timeEl = article.querySelector("time"); const timestamp = timeEl ? timeEl.textContent.trim() : ""; const authorUrl = authorLink ? authorLink.href : ""; if (content && postUrl) { posts.push({ author, content, url: postUrl, authorUrl, timestamp }); } } return posts; })()'
      }
    });
  ...
  async generateMarkdown() {
    ...
    const filename = 'weibo_posts_150_extracted.md';
  ...
  async start() {
    try {
      log('INIT', 'Starting fixed extraction');
...
new FixedExtractor().start().catch(console.error);
