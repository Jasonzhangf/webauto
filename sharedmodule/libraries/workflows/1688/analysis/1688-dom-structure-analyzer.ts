...
async function analyze1688ChatDOMStructure(): Promise<void> {
  ...
  try {
    ...
    const analysis: AnalysisResult = await page.evaluate(() => {
      ...
      // 查找contenteditable元素
      document.querySelectorAll('*').forEach((el: Element, index: number) => {
        if (el.contentEditable === 'true' || el.isContentEditable || el.getAttribute('contenteditable') === 'true') {
          ...
          result.analysis.inputArea.contenteditableElements.push({
            ...
            events: {
              ...
              onfocus: !!(el as HTMLElement).onfocus,
              onblur: !!(el as HTMLElement).onblur
            }
          });
        }
      });
      ...
      document.querySelectorAll('div, section, article').forEach((el: Element, index: number) => {
        const rect = el.getBoundingClientRect();
        ...
        if ((el as any)._reactInternalFiber || (el as any)._reactInternalInstance) {
          ...
          if (isVisible && hasInputCharacteristics) {
            result.analysis.inputArea.reactInputComponents.push({
              ...
              innerText: (el as HTMLElement).innerText?.substring(0, 100),
              rect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height },
              reactProps: (el as any)._reactInternalFiber ? {
                elementType: (el as any)._reactInternalFiber.elementType?.name || 'unknown',
                memoizedProps: Object.keys((el as any)._reactInternalFiber.memoizedProps || {})
              } : {}
            });
          }
        }
      ...
      // 查找包含"发送"文字的按钮
      document.querySelectorAll('button, [role="button"], a, div, span').forEach((el: Element, index: number) => {
        ...
        if (isVisible) {
          ...
          const buttonInfo = {
            ...
            events: {
              onclick: !!(el as HTMLElement).onclick,
              onmousedown: !!(el as HTMLElement).onmousedown,
              onmouseup: !!(el as HTMLElement).onmouseup
            },
            attributes: {
      ...
      ['header', 'main', 'section', 'article', 'aside', 'footer'].forEach(tagName => {
        document.querySelectorAll(tagName).forEach((el: Element, index: number) => {
          const rect = el.getBoundingClientRect();
      ...
      document.querySelectorAll('iframe').forEach((frame: Element, index: number) => {
        ...
        result.analysis.embeddedContent.iframes.push({
          ...
          rect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height },
          accessible: false,
          sameOrigin: (frame as HTMLIFrameElement).src === 'about:blank' || (frame as HTMLIFrameElement).src.startsWith(window.location.origin)
        });
      });
      ...
      inputContainerClasses.forEach(className => {
        document.querySelectorAll(`[class*="${className}"]`).forEach((el: Element, index: number) => {
          const rect = el.getBoundingClientRect();
    ...
    analysis.analysis.inputArea.contenteditableElements.forEach((el: any, i: number) => {
    ...
    analysis.analysis.sendButtons.exactSendButtons.forEach((btn: any, i: number) => {
