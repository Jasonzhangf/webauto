...
interface WorkflowEngine {
  executeWorkflow(workflowConfig: any, parameters: any): Promise<any>;
}

// 动态导入WorkflowEngine
async function getWorkflowEngine(): Promise<WorkflowEngine> {
  const WorkflowEngineClass = await import('./engine/WorkflowEngine.js');
  return new WorkflowEngineClass.default();
}

async function runDownloadWorkflow(): Promise<void> {
    try {
        // 检查命令行参数
