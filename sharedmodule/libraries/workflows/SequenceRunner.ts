...
class SequenceRunner {
  ...
  async runSequence(sequenceConfigPath: string): Promise<SequenceResult> {
    if (!existsSync(sequenceConfigPath)) {
      throw new Error(`序列配置不存在: ${sequenceConfigPath}`);
    }

    const cfg: SequenceConfig = JSON.parse(readFileSync(sequenceConfigPath, 'utf8'));
    if (!cfg || !Array.isArray(cfg.workflows) || cfg.workflows.length === 0) {
      throw new Error('无有效的序列工作流配置');
    }

    const results: any[] = [];
    let shared: Record<string, any> = cfg.sharedParameters || {};
