/**
 * Update LLMSwitchNode with Transformer Support
 * 更新LLMSwitchNode以支持Transformer
 *
 * 此脚本展示了如何更新LLMSwitchNode以集成Transformer功能
 */
...
class LLMSwitchNode extends BasePipelineNode {
  ...
  async handleTransformerConversion(sourceProtocol, targetProtocol, data) {
    // 获取源协议转换器
    const sourceTransformer = this.transformerManager.getTransformer(sourceProtocol);
    if (!sourceTransformer) {
      throw new Error(`Transformer for protocol '${sourceProtocol}' not found`);
    }

    // 获取目标协议转换器
    const targetTransformer = this.transformerManager.getTransformer(targetProtocol);
    if (!targetTransformer) {
      throw new Error(`Transformer for protocol '${targetProtocol}' not found`);
    }
