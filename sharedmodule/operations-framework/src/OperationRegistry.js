...
export class OperationRegistry {
  constructor(config = {}) {
    ...
    this.logger = {
      info: (message, data = {}) => console.log(`[OperationRegistry] INFO: ${message}`, data),
      warn: (message, data = {}) => console.warn(`[OperationRegistry] WARN: ${message}`, data),
      error: (message, data = {}) => console.error(`[OperationRegistry] ERROR: ${message}`, data),
      debug: (message, data = {}) => console.debug(`[OperationRegistry] DEBUG: ${message}`, data)
    };
  }
   ...
   * 注册操作子
   */
  register(OperationClass, metadata = {}) {
   ...
   * 动态发现类别
   */
  discoverCategory(OperationClass, metadata = {}, filePath = null) {
   ...
   * 从目录结构推断类别
   */
  inferCategoryFromDirectory(filePath) {
   ...
   * 从元数据推断类别
   */
  inferCategoryFromMetadata(OperationClass, metadata) {
    // 从操作子类名推断
   ...
   * 从类名提取类别
   */
  extractCategoryFromName(className) {
    // 移除Operation后缀
   ...
   * 创建操作子实例
   */
  createInstance(operationName, config = {}) {
   ...
   * 扫描单个文件中的操作子
   */
  async scanFile(filePath, classPattern) {
    ...
    try {
      ...
      for (const [exportName, exportValue] of Object.entries(module)) {
        if (this.isOperationClass(exportValue)) {
          try {
            // 传递文件路径用于动态类别发现
            const operationInfo = this.register(exportValue, {}, filePath);
   ...
   * 自动扫描多个目录
   */
  async autoScan(directories, options = {}) {
    const allOperations = [];
  ...
  getRecommendedCategories() {
    ...
    for (const op of this.operations.values()) {
      ...
      for (const keyword of keywords) {
        namePatterns.set(keyword.toLowerCase(), (namePatterns.get(keyword.toLowerCase()) || 0) + 1);
      }
    }
   ...
   * 验证操作子依赖
   */
  validateDependencies(operationName) {
    const operationInfo = this.get(operationName);
    const OperationClass = operationInfo.class;
