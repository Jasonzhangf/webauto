...
export class DataMergerOperation extends BaseOperation {
    ...
    try {
      ...
      const result = {
        ...
        metadata: {
          ...
          originalCount: finalParams.dataSources.reduce((sum: number, source: any) => sum + (Array.isArray(source) ? source.length : 1), 0),
      ...
      if (!finalParams.preserveOriginal) {
        result.metadata.dataSources = finalParams.dataSources.map((source: any, index: number) => ({
          index,
          type: Array.isArray(source) ? 'array' : 'object',
          count: Array.isArray(source) ? source.length : 1
        }));
      } else {
        (result.metadata as any).originalData = finalParams.dataSources;
      }
      ...
      this.log('error', 'Data merge failed', { error: (error as Error).message, executionTime });
  ...
  concatMerge(dataSources: any[], params: OperationConfig): any[] {
    let result: any[] = [];
  ...
  keyBasedMerge(dataSources: any[], params: OperationConfig): any[] {
    ...
    for (const source of dataSources) {
      ...
      for (const item of items) {
        ...
        if (resultMap.has(keyValue)) {
          ...
          const merged = this.resolveConflict(existing, item, params.conflictStrategy as string);
  ...
  joinMerge(dataSources: any[], params: OperationConfig): any[] {
    ...
    return primary.map(primaryItem => {
      ...
      for (const source of secondary) {
        const items = Array.isArray(source) ? source : [source];
        const matchingItem = items.find((item: any) => item[params.keyField] === keyValue);
  ...
  unionMerge(dataSources: any[], params: OperationConfig): any[] {
  ...
  resolveConflict(existing: any, newItem: any, strategy: string): any {
    switch (strategy) {
      case 'overwrite':
  ...
  combineFields(obj1: any, obj2: any): any {
    const result = { ...obj1 };
  ...
  deduplicate(items: any[], keyField: string): any[] {
    const seen = new Set();
  ...
  createItemSignature(item: any, keyField: string): string {
  ...
  transformFields(data: any[], transformations: DataTransformation): any[] {
  ...
  applyTransformation(value: any, transform: any): any {
  ...
  filterData(data: any[], conditions: Record<string, any>): any[] {
  ...
  matchesCondition(value: any, condition: any): boolean {
  ...
  applyOperator(value: any, operator: string, compareValue: any): boolean {
    switch (operator) {
  ...
  sortData(data: any[], sortBy: string, sortOrder: string): any[] {
...
export class DataFilterOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    try {
      ...
      this.log('error', 'Data filtering failed', { error: (error as Error).message, executionTime });
  ...
  selectFields(data: any[], includeFields: string[], excludeFields: string[]): any[] {
    return data.map(item => {
      const result: any = {};
  ...
  applyFilters(data: any[], filters: DataFilter[], logic: string, caseSensitive: boolean): any[] {
    return data.filter(item => this.matchesFilterConditions(item, filters, logic, caseSensitive));
  }

  matchesFilterConditions(item: any, filters: DataFilter[], logic: string, caseSensitive: boolean): boolean {
    if (logic === 'and') {
  ...
  matchesSingleCondition(item: any, filter: DataFilter, caseSensitive: boolean): boolean {
  ...
  ensureUnique(data: any[], field: string): any[] {
    const seen = new Set();
    return data.filter(item => {
  ...
  sampleData(data: any[], sampleSize: number): any[] {
...
export class DataTransformerOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    try {
      ...
      this.log('error', 'Data transformation failed', { error: (error as Error).message, executionTime });
  ...
  applyFieldTransformations(data: any[], transformations: DataTransformation): any[] {
    return data.map(item => {
      const result = { ...item };
  ...
  applyTransformation(value: any, transformation: any): any {
  ...
  addNewFields(data: any[], newFields: Record<string, any>): any[] {
    return data.map(item => {
      const result = { ...item };
  ...
  removeFields(data: any[], fieldsToRemove: string[]): any[] {
    return data.map(item => {
      const result = { ...item };
  ...
  applyStringTransformations(data: any[], params: OperationConfig): any[] {
    return data.map(item => {
      const result = { ...item };
  ...
  handleNullValues(data: any[], nullHandling: string, nullReplacement: string): any[] {
    return data.map(item => {
      const result: any = {};
  ...
  flattenArrays(data: any[], separator: string): any[] {
    return data.map(item => {
      const result: any = {};
  ...
  applyGroupBy(data: any[], groupBy: string, aggregations: Record<string, string>): any[] {
    ...
    for (const [groupKey, items] of groups) {
      const aggregated: any = { [groupBy]: groupKey };
      ...
      for (const [field, aggregation] of Object.entries(aggregations)) {
        ...
        switch (aggregation) {
          case 'sum':
            aggregated[field] = values.reduce((sum: number, val: any) => sum + Number(val), 0);
            break;
          case 'avg':
            aggregated[field] = values.reduce((sum: number, val: any) => sum + Number(val), 0) / values.length;
            break;
          case 'count':
  ...
  applyValidation(data: any[], validationRules: Record<string, ValidationRule[]>): any[] {
    return data.filter(item => {
  ...
  convertToFormat(data: any[], format: string): any {
    switch (format) {
      case 'object':
...
export class MarkdownConverterOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    try {
      ...
      this.log('error', 'Markdown conversion failed', { error: (error as Error).message, executionTime });
  ...
  generateFrontmatter(frontmatter: Record<string, any>): string {
  ...
  generateTable(data: any[], params: OperationConfig): string {
    if (data.length === 0) return 'No data to display';

    const headers = Object.keys(data[0]);
    const alignments = this.getTableAlignments(headers, params.tableAlignment as string);
  ...
  getTableAlignments(headers: string[], alignment: string): string[] {
  ...
  formatTableCell(value: any, params: OperationConfig): string {
    if (value === null || value === undefined) {
      return '';
    }
    ...
    if (typeof value === 'number') {
      return this.formatNumber(value, params.numberFormat as string);
    }

    if (value instanceof Date) {
      return this.formatDate(value, params.dateFormat as string);
    }
  ...
  generateList(data: any[], params: OperationConfig): string {
    if (data.length === 0) return 'No data to display';
    ...
    if (params.groupBy && data.length > 0) {
      // Group by field
      const groups = this.groupData(data, params.groupBy as string);
  ...
  generateItemList(items: any[], params: OperationConfig): string {
    const style = params.listStyle as string;
    let markdown = '';
  ...
  formatListItem(item: any, params: OperationConfig): string {
  ...
  generateCustomTemplate(data: any[], params: OperationConfig): string {
    ...
    for (const item of data) {
      let itemMarkdown = params.customTemplate as string;
  ...
  generateDefaultFormat(data: any[], params: OperationConfig): string {
    let markdown = '';
  ...
  generateSummarySection(data: any[], params: OperationConfig): string {
    if (data.length === 0) return '';
  ...
  generateDataSection(data: any[], params: OperationConfig): string {
    if (data.length === 0) return '';

    let markdown = '## Data\n\n';

    if (data.length <= (params.maxItemsPerSection as number)) {
      // Show all items
      markdown += this.generateTable(data, params);
    } else {
      // Show sample and summary
      const sample = data.slice(0, params.maxItemsPerSection as number);
  ...
  generateSection(data: any[], section: any, params: OperationConfig): string {
    let markdown = '';
  ...
  filterSectionData(data: any[], section: any): any[] {
    if (section.filter) {
  ...
  generateCodeBlock(data: any[], params: OperationConfig): string {
    const language = (params.codeBlockLanguage as string) || 'json';
    const code = JSON.stringify(data, null, 2);
    return `\`\`\`${language}\n${code}\n\`\`\``;
  }

  generateMetadata(data: any[], params: OperationConfig): string {
    const metadata = {
      generatedAt: new Date().toISOString(),
      itemCount: data.length,
  ...
  formatValue(value: any, params: OperationConfig): string {
    if (value === null || value === undefined) {
      return '';
    }
    ...
    if (typeof value === 'string') {
      ...
      if (value.startsWith('http://') || value.startsWith('https://')) {
        return (params.linkFormat as string).replace('text', value).replace('url', value);
      }
      return value;
    }
  ...
  groupData(data: any[], groupBy: string): Map<string, any[]> {
    const groups = new Map();
  ...
  generateDataSummary(data: any[]): Record<string, any> {
    if (data.length === 0) return {};
    ...
    for (const field of fields) {
      ...
      if (values.length > 0) {
        ...
        if (typeof values[0] === 'number') {
          summary[`${field} (avg)`] = (values.reduce((a: number, b: number) => a + b, 0) / values.length).toFixed(2);
