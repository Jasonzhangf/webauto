...
export class LinkExtractorOperation extends BaseOperation {
  constructor(config: OperationConfig = {}) {
    ...
    this.optionalParameters = {
      urlFilters: [] as string[],
      excludePatterns: [] as string[],
      domainFilter: '',
      linkTypes: ['all'] as string[],
      maxLinks: 100,
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    try {
      ...
      const rawLinks: LinkData[] = await page.evaluate((selector: string) => {
        const links = document.querySelectorAll(selector);
        return Array.from(links).map((link: HTMLAnchorElement) => {
          return {
            href: link.href,
      ...
      this.log('error', 'Link extraction failed', { error: (error as Error).message, executionTime });
  ...
  filterLinks(links: LinkData[], params: OperationConfig): LinkData[] {
    return links.filter(link => {
      ...
      // URL filters
      if (params.urlFilters && params.urlFilters.length > 0) {
        const matchesFilter = params.urlFilters.some((pattern: string) => {
      ...
      if (params.excludePatterns && params.excludePatterns.length > 0) {
        const matchesExclude = params.excludePatterns.some((pattern: string) => {
          const regex = new RegExp(pattern);
          return regex.test(link.href);
        });
        if (matchesExclude) {
      ...
      // Link type filter
      if (params.linkTypes && params.linkTypes.length > 0 && !params.linkTypes.includes('all')) {
  ...
  sortLinks(links: LinkData[], params: OperationConfig): LinkData[] {
  ...
  calculateLinkRelevance(link: LinkData): number {
    let score = 0;
  ...
  async enrichLinkMetadata(links: LinkData[], page: any): Promise<void> {
...
export class TextExtractorOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    try {
      ...
      const content: TextContent = await page.evaluate((params: any) => {
        const container = document.querySelector(params.selector) || document.body;

        // Remove excluded elements
        params.excludeSelectors.forEach((selector: string) => {
          container.querySelectorAll(selector).forEach((el: Element) => el.remove());
        });
      ...
      this.log('error', 'Text extraction failed', { error: (error as Error).message, executionTime });
...
export class ImageExtractorOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    try {
      ...
      const images: ImageData[] = await page.evaluate((params: any) => {
        const imageElements = document.querySelectorAll(params.selector);
        return Array.from(imageElements).map((img: HTMLImageElement) => {
      ...
      this.log('error', 'Image extraction failed', { error: (error as Error).message, executionTime });
  ...
  filterImages(images: ImageData[], params: OperationConfig): ImageData[] {
    return images.filter(img => {
      ...
      // Check format
      if (params.formats && params.formats.length > 0 && !params.formats.includes(img.format)) {
        return false;
      }
  ...
  sortImages(images: ImageData[], params: OperationConfig): ImageData[] {
  ...
  async enrichImageMetadata(images: ImageData[], page: any): Promise<void> {
  ...
  calculateImageRelevance(image: ImageData): number {
    let score = 0;
  ...
  countFormats(images: ImageData[]): Record<string, number> {
  ...
  async downloadImages(images: ImageData[], downloadPath: string): Promise<void> {
...
export class ContentExtractorOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    try {
      ...
      this.log('error', 'Content extraction failed', { error: (error as Error).message, executionTime });
  ...
  async extractMainContent(page: any, params: OperationConfig): Promise<string> {
    ...
    }, params.mainContentSelector);
  }

  async extractTextContent(page: any, mainContent: string, params: OperationConfig): Promise<TextContent | null> {
  ...
  async extractLinkContent(page: any, mainContent: string, params: OperationConfig): Promise<LinkData[] | null> {
  ...
  async extractImageContent(page: any, mainContent: string, params: OperationConfig): Promise<ImageData[] | null> {
  ...
  async extractStructuredData(page: any, mainContent: string, params: OperationConfig): Promise<any> {
  ...
  analyzeContent(content: any): any {
    const analysis: any = {
      readability: {},
      contentStats: {},
      recommendations: []
    ...
    if (content.text) {
      ...
      const avgSyllables = text.split(/\s+/).reduce((acc: number, word: string) => {
    ...
    if (content.links) {
      ...
      analysis.contentStats.externalLinks = content.links.filter((link: LinkData) => link.isExternal).length;
      analysis.contentStats.internalLinks = content.links.filter((link: LinkData) => link.isInternal).length;
    }

    // Analyze image content
    if (content.images) {
      analysis.contentStats.imageCount = content.images.length;
      analysis.contentStats.imagesWithAlt = content.images.filter((img: ImageData) => img.alt.length > 0).length;
