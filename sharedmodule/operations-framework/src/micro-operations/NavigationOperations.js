    ...
    try {
      ...
      while (attempt < finalParams.retryCount) {
        ...
        try {
          ...
          lastError = error;
          this.log('warn', `Navigation attempt ${attempt} failed`, { error: error.message });
...
export class ElementClickOperation extends BaseOperation {
  ...
  async waitForElementStable(element, timeout) {
    const startTime = Date.now();
  ...
  async getElementInfo(element) {
    try {
      ...
      this.log('warn', 'Failed to get element info', { error: error.message });
      return {
        error: error.message
      };
    }
...
export class FormFillOperation extends BaseOperation {
  ...
  async clearForm(form) {
    ...
    for (const input of inputs) {
      ...
      try {
        ...
        this.log('warn', 'Failed to clear form field', { error: error.message });
      }
    }
  }
  
  async fillFormFields(form, formData, options) {
    const results = {
      success: [],
      failed: [],
      skipped: []
  ...
  getFieldSelector(fieldName, fieldMapping) {
  ...
  async fillFormField(field, value, options) {
  ...
  async validateFormFields(form, formData) {
    const results = {
      valid: [],
      invalid: [],
      required: []
    ...
    for (const [fieldName, expectedValue] of Object.entries(formData)) {
      try {
        ...
        const required = await field.evaluate(el => el.hasAttribute('required'));
        if (required) {
  ...
  validateFieldValue(actual, expected) {
  ...
  async submitForm(form, page, options) {
    try {
