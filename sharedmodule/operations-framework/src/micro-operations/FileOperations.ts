...
export class JsonFileSaverOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    const finalParams = validation.finalParams as JsonFileConfig & { data: any; filePath: string };
    ...
    try {
      ...
      if (finalParams.backup && await this.fileExists(finalParams.filePath)) {
        await this.createBackup(finalParams.filePath, finalParams.backupSuffix || '.backup');
      }
      ...
      return {
        ...
        result: {
          ...
          backupCreated: finalParams.backup && await this.fileExists(finalParams.filePath + (finalParams.backupSuffix || '.backup'))
        },
        metadata: {
  ...
  addMetadata(data: any, fields: string[]): any {
    const metadata: any = {};
  ...
  async createBackup(filePath: string, suffix: string): Promise<void> {
...
export class MarkdownFileSaverOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    const finalParams = validation.finalParams as MarkdownConfig & { data: any; filePath: string };
  ...
  async generateMarkdown(data: any, params: MarkdownConfig & { data: any; filePath: string }): Promise<string> {
    const dataForConversion = Array.isArray(data) ? data : [data];

    // Use MarkdownConverterOperation to generate content
    const { MarkdownConverterOperation } = await import('./DataProcessingOperations');
    ...
    if (params.includeTableOfContents) {
      markdown = this.addTableOfContents(markdown, params.tableOfContentsTitle, params.maxDepth || 3);
    }
  ...
  addTableOfContents(markdown: string, title: string | undefined, maxDepth: number): string {
  ...
  addStatistics(markdown: string, data: any[], params: MarkdownConfig): string {
  ...
  generateDataStatistics(data: any[]): Record<string, any> {
...
export class CsvFileSaverOperation extends BaseOperation {
  ...
  async execute(context: OperationContext, params: OperationConfig = {}): Promise<OperationResult> {
    ...
    const finalParams = validation.finalParams as CsvConfig & { data: any; filePath: string };
  ...
  async prepareData(data: any, params: CsvConfig): Promise<any[]> {
    // Ensure data is an array
    let dataArray = Array.isArray(data) ? data : [data];

    // Limit rows if specified
    if (params.maxRows && params.maxRows > 0 && dataArray.length > params.maxRows) {
  ...
  formatValue(value: any, params: CsvConfig): string {
    ...
    if (typeof value === 'number') {
      return this.formatNumber(value, params.numberFormat || 'decimal');
    }

    if (typeof value === 'boolean') {
      return this.formatBoolean(value, params.booleanFormat || 'truefalse');
    }

    if (value instanceof Date) {
      return this.formatDate(value, params.dateFormat || 'YYYY-MM-DD');
    }
  ...
  formatBoolean(value: boolean, format: string): string {
    switch (format) {
      case 'yesno':
        return value ? 'Yes' : 'No';
      case '10':
  ...
  generateCsvContent(data: any[], params: CsvConfig): string {
    if (data.length === 0) {
      return '';
    }

    // Get headers
    let headers = params.headers && params.headers.length > 0 ? params.headers : Object.keys(data[0]);
  ...
  formatCsvRow(rowData: any[], params: CsvConfig): string {
