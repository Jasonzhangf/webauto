...
class EventDrivenCookieManager {
    ...
    setupEventDrivenWorkflow() {
        ...
        this.workflowEngine.addRule({
            ...
            when: 'cookie:badge:detection:complete',
            condition: (data) => {
                // 只有当徽章检测成功且登录确认时才执行
    ...
    async validateCookies() {
        ...
        try {
            ...
            const hasEssential = essentialCookies.some(name => cookies.some(cookie => cookie.name === name));
     ...
     * 获取可见元素数量
     */
    async getVisibleElementsCount(elements) {
        let visibleCount = 0;
     ...
     * 对比Cookie变化
     */
    async compareCookies(oldCookies) {
        ...
        for (const [name, cookie] of newCookieMap) {
            if (!oldCookieMap.has(name)) {
                comparison.added.push(cookie);
            }
        }
        // 找出删除的Cookie
        for (const [name, cookie] of oldCookieMap) {
            if (!newCookieMap.has(name)) {
                comparison.removed.push(cookie);
            }
        }
        // 找出修改的Cookie
        for (const [name, newCookie] of newCookieMap) {
            const oldCookie = oldCookieMap.get(name);
            if (oldCookie && newCookie.value !== oldCookie.value) {
                comparison.modified.push({ old: oldCookie, new: newCookie });
            }
        }
        // 找出未变化的Cookie
        for (const [name, newCookie] of newCookieMap) {
            const oldCookie = oldCookieMap.get(name);
            if (oldCookie && newCookie.value === oldCookie.value) {
                comparison.unchanged.push(newCookie);
            }
        }
        return comparison;
    }
...
//# sourceMappingURL=event-driven-cookie-manager.js.map