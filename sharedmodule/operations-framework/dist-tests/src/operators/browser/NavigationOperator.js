...
export class NavigationOperator extends NonPageOperator {
    ...
    // 核心导航方法
    async navigateTo(url, waitUntil, timeout) {
        try {
            // 验证URL格式
    ...
    updateHistory(url) {
        // 如果不在历史记录的末尾，清除后面的记录
        if (this._navigationHistory.currentIndex < this._navigationHistory.history.length - 1) {
            this._navigationHistory.history = this._navigationHistory.history.slice(0, this._navigationHistory.currentIndex + 1);
        }
        // 添加新URL
    ...
    // 模拟导航（实际实现需要与浏览器实例集成）
    async simulateNavigation(url, waitUntil, timeout, isRefresh) {
        // 这里是简化版本，实际实现需要：
        // 1. 与真实的浏览器页面实例集成
        // 2. 等待页面加载完成
        // 3. 处理各种加载状态
...
//# sourceMappingURL=NavigationOperator.js.map