...
export class StateOperator extends NonPageOperator {
    ...
    // 核心状态管理方法
    async setState(key, value, namespace, ttl) {
        try {
    ...
    async retrieveState(key, namespace) {
        try {
            const fullKey = this.getFullKey(key, namespace);
    ...
    async incrementState(key, incrementBy = 1, namespace) {
        try {
            const fullKey = this.getFullKey(key, namespace);
            const entry = this._stateStore.get(fullKey);
    ...
    async decrementState(key, decrementBy = 1, namespace) {
        try {
            const fullKey = this.getFullKey(key, namespace);
            const entry = this._stateStore.get(fullKey);
            let currentValue = entry?.value || 0;
    ...
    async watchState(key, namespace, callback) {
        try {
            const fullKey = this.getFullKey(key, namespace);
    ...
    // 清理方法
    async clearExpiredStates() {
        return this.cleanupExpired();
    }
    async getStateNamespaces() {
        try {
...
//# sourceMappingURL=StateOperator.js.map