...
export class CookieOperator {
    ...
    async loadCookies(filePath) {
        try {
            ...
            this._currentCookies = data.cookies.map((cookie) => ({
                name: cookie.name,
                value: cookie.value,
                domain: cookie.domain,
                path: cookie.path,
    ...
    async setCookie(name, value, domain) {
        ...
        const existingIndex = this._currentCookies.findIndex(c => c.name === name && c.domain === cookie.domain);
        if (existingIndex >= 0) {
    ...
    async getCookie(name, domain) {
        let cookie = this._currentCookies.find(c => c.name === name && (!domain || c.domain === domain));
        if (!cookie) {
            return {
                success: false,
    ...
    async deleteCookie(name, domain) {
        ...
        this._currentCookies = this._currentCookies.filter(c => !(c.name === name && (!domain || c.domain === domain)));
    ...
    async exportCookies(filePath, format = 'json') {
        try {
            const absolutePath = path.resolve(filePath);
            const dirPath = path.dirname(absolutePath);
            await fs.mkdir(dirPath, { recursive: true });
