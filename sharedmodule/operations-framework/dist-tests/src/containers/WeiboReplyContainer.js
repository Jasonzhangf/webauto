...
exports.WeiboReplyContainer = void 0;
const BaseSelfRefreshingContainer_1 = require("./BaseSelfRefreshingContainer");
const UniversalOperator_1 = require("../core/UniversalOperator");
// ==================== 容器实现 ====================
class WeiboReplyContainer extends BaseSelfRefreshingContainer_1.BaseSelfRefreshingContainer {
    constructor(config) {
        super({
    ...
    async executeDynamicOperation(page, operation, params) {
        switch (operation.action) {
            ...
            default:
                return UniversalOperator_1.OperationResult.failure(`不支持的操作: ${operation.action}`);
        }
    }
    // ==================== 核心刷新逻辑 ====================
    async performRefresh(trigger) {
        ...
        try {
            ...
            if (!stateUpdate.exists) {
                return UniversalOperator_1.OperationResult.success({
                    action: 'refresh',
            ...
            return UniversalOperator_1.OperationResult.failure(`刷新失败: ${error.message}`, error);
        }
    }
    // ==================== 回复数据提取 ====================
    async extractReplies(page) {
        try {
            ...
            return UniversalOperator_1.OperationResult.success(replies);
        }
        catch (error) {
            return UniversalOperator_1.OperationResult.failure(`回复提取失败: ${error.message}`, error);
        }
    }
    async updateReplyData(newReplies) {
    ...
    // ==================== 自动展开回复 ====================
    shouldAutoExpandReplies(trigger) {
    ...
    async executeExpandReplies(page, operation) {
        try {
            ...
            return UniversalOperator_1.OperationResult.failure(`展开回复失败: ${error.message}`, error);
        }
    }
    async executeClickExpand(page, operation) {
        try {
            ...
            return UniversalOperator_1.OperationResult.failure(`点击展开失败: ${error.message}`, error);
        }
    }
    async executeScrollToReplies(page, operation) {
        try {
            ...
            return UniversalOperator_1.OperationResult.failure(`滚动到回复区域失败: ${error.message}`, error);
        }
    }
    async getCurrentReplyCount(page) {
        try {
            const selectors = this.config.replySelectors || [
                '.Reply_item',
                '.Comment_replies .Reply_item',
    ...
    inferExpandActionFromText(text) {
        const actionMap = {
            '展开回复': 'expand_replies',
            '查看回复': 'expand_replies',
            '展开': 'expand',
...
exports.WeiboReplyContainer = WeiboReplyContainer;
exports.default = WeiboReplyContainer;
