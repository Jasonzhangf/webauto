...
export class IntelligentScrollOperation extends BaseAtomicOperation {
    ...
    async execute(context, params = {}) {
        const { page } = context;
        const { scrollContainer = 'body', targetPosts = 50, currentPosts = 6, postsPerScroll = { min: 2, expected: 5, max: 10 }, scrollStrategy = 'dynamic-adaptive', maxScrollAttempts = 100, baseScrollStep = 800, adaptiveStepRange = { min: 300, max: 1200 }, scrollDelays = { min: 800, max: 2000, adaptive: true }, completionCriteria = {
            targetReached: 50,
            maxNoNewPosts: 8,
     ...
     * 执行微博专用滚动策略
     */
    async executeWeiboScrollStrategy(page, options) {
        const { scrollContainer, targetPosts, currentPosts, postsPerScroll, maxScrollAttempts, baseScrollStep, adaptiveStepRange, scrollDelays, completionCriteria, detection } = options;
        let scrollCount = 0;
     ...
     * 执行微博专用滚动
     */
    async performWeiboScroll(page, scrollContainer, stepSize) {
     ...
     * 自适应微博延迟
     */
    async adaptiveWeiboDelay(delayConfig, scrollCount, totalNewPosts) {
     ...
     * 适应微博滚动参数
     */
    adaptWeiboScrollParameters(params) {
        const { newPostsThisScroll, currentStep, scrollCount, postsPerScroll, adaptiveStepRange, targetPosts, newPostCount } = params;
     ...
     * 检查微博完成条件
     */
    async checkWeiboCompletionConditions(params) {
        const { newPostsThisScroll, newPostCount, targetPosts, scrollCount, completionCriteria } = params;
        // 1. 达到目标帖子数
    ...
    calculateScrollEfficiency() {
        if (this.scrollStats.totalScrolls === 0)
            return 0;
     ...
     * 初始化内容观察器
     */
    async initializeContentObserver(page, scrollContainer) {
        await page.evaluate((container) => {
            ...
            // 存储observer
            window.weiboContentObserver = observer;
        }, scrollContainer);
    }
    /**
     * 清理资源
    ...
    async cleanup(page) {
        try {
            await page.evaluate(() => {
                if (window.weiboContentObserver) {
                    window.weiboContentObserver.disconnect();
                    delete window.weiboContentObserver;
                }
            });
        }
        catch (error) {
...
//# sourceMappingURL=IntelligentScrollOperation.js.map