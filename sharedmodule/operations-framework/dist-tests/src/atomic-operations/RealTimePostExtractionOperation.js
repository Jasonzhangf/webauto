...
export class RealTimePostExtractionOperation extends BaseAtomicOperation {
    ...
    async execute(context, params = {}) {
        const { page } = context;
        const { extractionTriggers = ['after-scroll', 'mutation-detected', 'time-interval'], extractionInterval = 3000, linkSelectors = [
        ...
        }, extractionMode = 'incremental', batchSize = 10, maxExtractions = 100, timeout = 300000 } = params;
     ...
     * 提取微博帖子
     */
    async extractWeiboPosts(page, params) {
        const { linkSelectors, validation, extractionMode, batchSize } = params;
        ...
        const extractedData = await page.evaluate((selectors) => {
            ...
            // 去重
            const uniqueLinks = allLinks.filter((link, index, self) => index === self.findIndex(l => l.url === link.url));
            return {
     ...
     * 处理提取的链接
     */
    async processExtractedLinks(extractedData, validation) {
        const newPosts = [];
     ...
     * 检查内容变化
     */
    async checkForContentChanges(page) {
        try {
     ...
     * 处理滚动完成事件
     */
    async handleScrollPerformed(data) {
        // 滚动后自动触发提取
    ...
    calculateExtractionEfficiency() {
        if (this.extractionStats.totalExtractions === 0)
            return 0;
     ...
     * 获取完成原因
     */
    getCompletionReason(extractionCount, maxExtractions, timeout, startTime) {
        if (this.extractionStats.uniquePosts >= 50) {
...
//# sourceMappingURL=RealTimePostExtractionOperation.js.map