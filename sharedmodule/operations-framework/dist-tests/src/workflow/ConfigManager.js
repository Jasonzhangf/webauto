...
export class ConfigManager {
    constructor() {
        this._workflowSchemas = new Map();
        this._operatorLibraries = new Map();
     ...
     */
    async loadOperatorLibrary(filePath) {
        try {
            const absolutePath = path.resolve(filePath);
            const content = await fs.readFile(absolutePath, 'utf-8');
     ...
     */
    createTemplateWorkflow(template) {
        switch (template) {
            case 'basic-browser':
                return {
     ...
     * Validate operator library structure
     */
    validateOperatorLibrary(library) {
        const errors = [];
        const warnings = [];
