...
import { containerConfigManager } from '../examples/container-integration-configs.js';
...
export interface TestCase {
  name: string;
  type: 'unit' | 'integration' | 'performance' | 'error';
...
export class ContainerSystemTestRunner {
  ...
  private async runPerformanceTest(testCase: TestCase): Promise<any> {
    ...
    try {
      ...
      const timeStdDev = Math.sqrt(
        refreshTimes.reduce((sum, time) => sum + Math.pow(time - averageTime, 2), 0) / refreshTimes.length
      );
  ...
  private async runErrorTest(testCase: TestCase): Promise<any> {
  ...
  private validateTestResult(actual: any, expected: TestCase['expected']): { success: boolean; details?: any } {
  ...
  private ratePerformance(averageTime: number): 'excellent' | 'good' | 'fair' | 'poor' {
    if (averageTime < 100) return 'excellent';
    if (averageTime < 500) return 'good';
    if (averageTime < 1000) return 'fair';
    return 'poor';
  }
  ...
  private generateTestReport(): TestSuiteResult {
    ...
    if (successRate >= 95 && averageExecutionTime < 500) performance = 'excellent';
    else if (successRate >= 80 && averageExecutionTime < 1000) performance = 'good';
    else if (successRate >= 60) performance = 'fair';
...
export const containerSystemTestConfig: TestConfig = {
...
export async function runContainerSystemTests(): Promise<TestSuiteResult> {
  console.log('🚀 开始容器系统测试');
  console.log('==========================');
