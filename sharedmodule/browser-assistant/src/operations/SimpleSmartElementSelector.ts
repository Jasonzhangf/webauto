...
export class SmartElementSelector extends BaseBrowserModule {
  ...
  async selectElement(page: Page, context: ElementContext): Promise<ElementSelection> {
    try {
      ...
      let selection: ElementSelection | null = null;
  ...
  async selectAllElements(page: Page, context: ElementContext): Promise<ElementSelection[]> {
    try {
      let selections: ElementSelection[] = [];
   ...
   * 基于类型选择元素
   */
  private async selectByType(page: Page, type: string): Promise<ElementSelection> {
    try {
      // 常见类型的CSS选择器
   ...
   * 基于文本选择元素
   */
  private async selectByText(page: Page, text: string): Promise<ElementSelection> {
    try {
   ...
   * 基于属性选择元素
   */
  private async selectByAttributes(page: Page, attributes: Record<string, string>): Promise<ElementSelection> {
    try {
      // 构建属性选择器
   ...
   * 基于ARIA role选择元素
   */
  private async selectByRole(page: Page, role: string): Promise<ElementSelection> {
    try {
   ...
   * 回退选择策略
   */
  private async selectFallback(page: Page): Promise<ElementSelection> {
    try {
      // 尝试找到第一个可交互的元素
   ...
   * 生成缓存键
   */
  private getCacheKey(page: Page, context: ElementContext): string {
    const url = page.url();
   ...
   * 获取元素选择建议
   */
  async getElementSuggestions(page: Page, context: ElementContext): Promise<string[]> {
