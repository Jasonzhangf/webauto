...
let procs = { workflow: null, vision: null };
...
app.get('/health', async (req, res) => {
  try {
    ...
    if (agg.services.workflow.status !== 'ok' || agg.services.vision.status !== 'ok') agg.status = 'degraded';
    return res.json(agg);
  } catch (e) {
...
app.get('/ports', async (req, res) => {
  const ports = [CONFIG.ports.orchestrator, CONFIG.ports.workflow, CONFIG.ports.vision, CONFIG.ports.visionPy];
