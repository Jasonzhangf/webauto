# XHS Detail 评论+点赞状态机（v2026-03-19）
...
[S3 HARVEST_COMMENTS]
  ...
  - 滚动焦点规则（2026-03-19）：
      - recovery/coverage_retry 的第一次滚动必须执行 focus click（skipFocusClick=false）
      - 避免滚动容器未聚焦导致 scrollTop 不变

  - 进度锚点:
      progress = (collectedRows.length 增长) OR (scrollSignature 变化)
      churn-only(可见评论顺序抖动) != progress
  - 覆盖率策略（2026-03-19）：
      - 目标覆盖率：visibleCount / expectedCommentsCount >= 0.9
      - 当 reached_bottom 且覆盖率不足时触发 coverage_retry
      - coverage_retry 流程：
          a) scroll 回顶部（确保容器获得 focus）
          b) 重新 expand replies
          c) 重置滚动状态后继续向下滚动
      - coverage_retry 超过 maxCoverageRetries（默认 2）后仍不足 -> coverage_insufficient
      - coverage_retry 必须记录 coverage_retry_start / complete 事件

  - 退出条件:
      a) reached_bottom（仅在 coverage >= 0.9 时允许）
      b) coverage_satisfied / coverage_insufficient（最大轮次或重试耗尽）
      c) tab_comment_budget_reached(默认 50)
      d) scroll_stalled_after_recovery
      e) comments_empty
  - 新增锚点约束（2026-03-13）：
