...
PORT_PY_VISION=8899
PYTHON_BIN=python3
PY_SERVICE_ENTRY=runtime/vision/ui-recognition/python-service/ui_ins_server.py
```

可选的 Python 服务入口（通过设置 `PY_SERVICE_ENTRY` 覆盖）：
- 官方Qwen/PT版（默认）：`runtime/vision/ui-recognition/python-service/ui_ins_server.py`
- MLX实验版（Apple Silicon，仅在必要时启用）：`runtime/vision/ui-recognition/python-service/mlx_ui_server.py`
- MLX LLaVA 版（Apple Silicon，真实视觉推理）：`runtime/vision/ui-recognition/python-service/mlx_llava_server.py`
...
- 启动：将 `PY_SERVICE_ENTRY` 设为 `runtime/vision/ui-recognition/python-service/mlx_llava_server.py`
