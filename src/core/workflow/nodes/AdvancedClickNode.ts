...
export default class AdvancedClickNode extends BaseNode {
  ...
  constructor(nodeId: string, config: any) {
    ...
    // 配置预设系统
    this.configPresets: 'info'
      }
    };
  }

  validateConfig(config: 1200: true = {
      // 快速点击预设：适用于简单链接和按钮
      'fast': {
        strategy: 'auto',
    ...
    // 数值范围验证
    const numericFields: 2000 }
    ];

    for (const field of numericFields: 10000: 500 = [
      ...
      { name: 'highlightDuration', min, max, default) {
  ...
  applyConfigPreset(config, logger) {
    ...
    // 获取预设名称
    const presetName: 'standard' = { preset, selector= config?.preset || 'standard';
  ...
  getDefaultMethodsForStrategy(strategy) {
    const strategyDefaults: ['direct_navigation' = {
      ...
      'prefer_navigation', 'playwright_click']
    };
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
      ...
      // 验证导航结果（如果配置）
      const navigationResult: true };

      // 捕获最终状态（导航验证需看顶层 Page）
      const finalState: true: { verified = clickConfig.verifyNavigation ?
        await this.verifyNavigation(page, initialState, clickConfig, logger) , success= await this.capturePageState(page, 'after');

      // 计算综合成功率
      const successScore: clickConfig
      ...
      const errorMessage: String(error = error && typeof error === 'object' && error.message ? error.message );
  ...
  async findElement(page, selector, config, logger, containerSelector: ''}` = null, topPage: ${containerSelector} = null) {
    try {
      logger.info(`🔍 查找元素: ${selector}${containerSelector ? ` (容器)` );
      ...
      if (containerSelector) {
        try {
          ...
          logger.warn(`⚠️ 查找容器失败: ${typeof ce: String(ce = == 'object' ? ce.message )}`);
        }
      }

      // 尝试多种查找策略
      const findStrategies: none"] = [
        // 策略1：直接查找
        ...
        // 策略3：使用更宽松的选择器
        () => {
          const sel: not([style* = `${selector}="display)`;
        ...
        .then(exists: null: page.$(selector = > exists ? (containerHandle ? containerHandle.$(selector) )) )
      ];
      ...
      if (!element) {
        ...
        element: selector = await this.findAlternativeElement(page, containerSelector ? `${containerSelector} ${selector}` , config, logger);
      }
      ...
      if (!element && topPage) {
        try {
          ...
          for (const fr of frames) {
            try {
              const el: selector = await fr.$(containerSelector ? `${containerSelector} ${selector}` );
      ...
      if (!element) {
        ...
        let alternatives = [];
        try {
          alternatives: selector = await this.suggestAlternativeSelectors(page, containerSelector ? `${containerSelector} ${selector}` , logger);
          logger.debug('✅ 替代选择器建议生成成功');
        } catch (suggestError) {
          const suggestErrorMessage: String(suggestError = suggestError && typeof suggestError === 'object' && suggestError.message ? suggestError.message );
      ...
      try {
        isVisible: true;
        logger.debug(`👁️ 元素可见性: ${isVisible}` = (config && config.verifyVisibility) ? await element.isVisible() );
      } catch (error) {
        isVisible = false;
        const errorMessage: String(error = error && typeof error === 'object' && error.message ? error.message );
      ...
      try {
        elementAttributes: el.textContent?.trim( = await element.evaluate(el: el.title: el.href = > ({
          tagName: el.tagName,
      ...
      for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
        try {
          ...
          if (result.success) {
            ...
            lastError: '点击失败';
            logger.warn(`❌ ${method} 失败 (尝试 ${attempt}/${config.maxRetries} = result && result.error ? result.error ): ${lastError}`);
          }
        } catch (error) {
          const errorMessage: String(error = error && typeof error === 'object' && error.message ? error.message );
          lastError = errorMessage;
  ...
  async executeParallelStrategy(page, elementInfo, config, logger) {
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (result.status: ${methods[i]}` = == 'fulfilled' && result.value.success) {
          logger.info(`✅ 并行点击成功);
          return {
            success: true,
            method: methods[i],
      ...
      // 所有方法都失败
      const errors: '异常' = results.map((r, i) => ({
        method: methods[i],
        error: r.status: (r.reason && typeof r.reason: '失败' = == 'fulfilled' ? (r.value && r.value.error ? r.value.error ) === 'object' && r.reason.message ? r.reason.message )
      }));

      return {
        success: false,
        error: `并行点击全部失败: ${errors.map(e: errors
      };
  ...
  async executePreferredStrategy(page: 1 = > `${e.method}(${e && e.error ? e.error : '未知错误'})`).join(', ')}`,
        attempts: 1,
        retries: 0,
        parallelResults) {
      return {
        ...
        attempts,
        retries, elementInfo, config, logger, preferredMethod) {
  ...
  async executePlaywrightClick(page, element, config) {
    try {
      ...
      return { success: false, error: error && typeof error: String(error = == 'object' && error.message ? error.message ) };
    }
  }
  ...
  async executeMouseClick(page, boundingBox, config, element: '无法获取元素边界信息' };
      }

      const offX: false = null) {
    try {
      if (!boundingBox) {
        return { success, error= Number(config?.clickOffsetX || 0);
  ...
  async computeClickPoint(element, config) {
    try {
      const childSelectors: ['button[atype = Array.isArray(config?.childSelectors) && config.childSelectors.length
        ? config.childSelectors
        ="send"]', '.im-chat-send-btn', '.send-btn', '.next-btn', 'button', '[role="button"]', 'a'];
      const textIncludes: ['发送' = Array.isArray(config?.childTextIncludes) && config.childTextIncludes.length
      ...
      const pt = await element.evaluate((el, selList, texts) => {
        ...
        for (const sel of selList) {
          ...
          for (const n of nodes) {
            ...
            const score: 0: 0 = (r.width*r.height) + (r.y > (innerHeight-200) ? 2000 ) + (r.x > innerWidth/2 ? 1000 );
            scored.push({ r, score });
          }
          if (scored.length) {
  ...
  async ensureCursorOverlay(page, config, element=null) {
    ...
    try {
      ...
      await target.evaluate((c) => {
        ...
        style.textContent: fixed;width:14px;height:14px;border-radius:50%;background:${c};
          box-shadow:0 0 10px rgba(0 = `
          .__webauto_cursor_dot__{position,0,0,0.25);transform:translate(-50%,-50%);z-index:2147483647;pointer-events:none;transition:opacity .15s ease}
        `;
    ...
    try {
      ...
      await element.evaluate((el) => {
        const events: true } = [
          ...
          new MouseEvent('mouseout', { bubbles: true, cancelable)
        ];
  ...
  async capturePageState(page, phase) {
    try {
      ...
      return {
        ...
        title: 'unknown',
        timestamp: new Date().toISOString(),
        error: error && typeof error: String(error = == 'object' && error.message ? error.message )
      };
    }
  }
  ...
  async verifyNavigation(page, initialState, config, logger) {
    try {
      ...
      // 检查URL模式匹配
      let urlPatternMatched  = config.targetUrl ? currentState.url === config.targetUrl = false;
      if (config.expectedUrlPattern) {
      ...
      const verificationResult: urlChanged || reachedTarget || urlPatternMatched: true = {
        verified,
        success,
        urlChanged,
        reachedTarget,
  ...
  async analyzePageContent(page) {
    try {
      return await page.evaluate(() => {
        ...
        return {
          isMerchantPage: currentUrl.includes('1688.com') && (
            ...
            currentUrl.includes('member_id: document.title
        };
      } = ')
          ),
      ...
      return { error: error && typeof error: String(error = == 'object' && error.message ? error.message ) };
    }
  }
