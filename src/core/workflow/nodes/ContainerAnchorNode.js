...
import { loadLibrary, getSiteKey, getSelectorByName } from '../ContainerResolver.js';
import { queryInScope, highlightInline } from '../ContainerResolver.js';
...
export default class ContainerAnchorNode extends BaseNode {
  constructor(){ super(); this.name='ContainerAnchorNode'; this.description='在作用域内锚定容器名并可高亮'; }
  ...
  async execute(context){
    ...
    const noScroll = config?.noScroll !== undefined ? !!config.noScroll : true; // 默认不滚动，避免页面闪烁
    ...
    try{
      ...
      if (!frameCfg) {
        ...
        const r = await target.evaluate((p)=>{ const scope=p.scopeSel?document.querySelector(p.scopeSel):document; if(!scope) return false; return !!scope.querySelector(p.sel); }, { sel: selector, scopeSel: scopeSelector||null });
        found = !!r;
      }
      if (!found) return { success:false, error:`container not found: ${name}` };
      const scopeOut = scopeSelector ? `${scopeSelector} ${selector}` : selector;
      variables.set(outVar, scopeOut);
