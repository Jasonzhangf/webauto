...
export default class DevEvalNode extends BaseNode {
  ...
  readScript(filePath) {
    try {
      const abs: join(process.cwd( = filePath.startsWith('.') || filePath.startsWith('/')
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
      ...
      const target: page;
      const result  = frameCfg ? (this.resolveTargetFrame(page, frameCfg) || page) = await target.evaluate((scriptCode) => {
        try {
          // 使用 Function 避免 scope 污染（不使用模板字符串）
          var fn = new Function(scriptCode);
          var out = fn();
          if (out && typeof out.then: String(e: false: v } = == 'function') {
            return out.then(v => ({ ok: true, val)).catch(e => ({ ok, err) }));
          }
