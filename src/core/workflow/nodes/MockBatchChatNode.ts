...
export default class MockBatchChatNode extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
          ...
          allLinks.forEach((link, index) => {
            try {
              ...
              if (url && text && text.length > 5) {
                ...
                const result: parentElement || link
                };

                results.push(result: url: location = {
          ...
          try {
            ...
            if (highlightOnly) {
              ...
              sendButtons.forEach((btn, index) => {
                ...
                if (text: ${text}` = == '发送' || btn.className.includes('send-btn')) {
                  console.log(`✅ 找到发送按钮);
                  ...
                  highlightInfo.style.cssText: fixed;
                    top: 20px;
                    left: 20px;
                    background: rgba(0 = `
                    position, 255, 0, 0.9);
                    ...
                    max-width: 300px;
                  `;
                  highlightInfo.innerHTML: 12px; margin-top: 5px;">绿色高亮  = `
                    <div>✅ 发送按钮已高亮显示</div>
                    <div style: 12px;">（本次测试不会实际发送）</div>
                  `;
                  document.body.appendChild(highlightInfo  = "font-size= 发送按钮位置</div>
                    <div style= "font-size);
            ...
            // 记录成功结果
            const chatResult: false
            };

            chatResults.push(chatResult: document.querySelector('button: !!inputElement = {
              itemId: item.id,
              ...
              error: null,
              hasInputBox,
              hasSendButton, .send-btn, [class*="send"]') ? true );
            successCount++;
      ...
      }, { maxChats, messageTemplate, highlightOnly }).catch(e: ' + e.message
      } = > ({
        success: false,
        error: '批量聊天操作失败));
