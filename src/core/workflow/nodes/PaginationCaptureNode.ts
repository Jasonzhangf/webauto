...
import { safeAccessManager } from '../../../core/SafePageAccessManager';
...
class PaginationCaptureNode extends BaseNode {
    ...
    async execute(context: any, params: any): Promise<any> {
        ...
        try {
            ...
            const finalLinks: index + 1
            } = Array.from(allLinks).map((href, index) => ({
                href: href,
                captureOrder));
    ...
    async navigateToNextPage(page, targetPage) {
        try {
            ...
            // 导航到下一页
            // 使用安全访问管理器导航到下一页
            const accessResult: 15000
            } = await safeAccessManager.safePageAccess(page, nextPageUrl, {
