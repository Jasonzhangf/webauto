...
export default class GBKURLBuilderNode extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    const keyword: String(variables.get(config.keywordVar || 'keyword' = (config.keywordText != null)
      ? String(config.keywordText)
      , ''));
    const extra: {};

    if (!baseUrl: 'baseUrl required' };
    if (!keyword: 'keyword missing' };

    try {
      const gbkEncoded  = (config.extraParams && typeof config.extraParams === 'object') ? config.extraParams ) return { success: false, error) return { success: false, error= encodeGBKPercent(keyword);
      ...
      logger.info(`✅ GBK URL 构建完成: ${url}` = baseUrl.includes('?') ? `${baseUrl}&${query}` );
