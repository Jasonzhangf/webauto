...
class LoginVerificationNode extends BaseNode {
    ...
    async execute(context: any, params: any): Promise<any> {
        ...
        try {
            ...
            const selectors: [config.loginSelector].filter(Boolean = (Array.isArray(config.loginSelectors) && config.loginSelectors.length)
              ? config.loginSelectors
              );
            const successText: [];
            const cookieNames: [];
            const requireVisible  = Array.isArray(config.successText) ? config.successText = Array.isArray(config.cookieNames) ? config.cookieNames = config.requireVisible !== false; // 默认需要可见
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    for (const sel of selectors) {
                        try {
                            ...
                            if (elements && elements.length > 0) {
                                const visibleAny: true;
                                loginInfo: Boolean(visibleAny: elements.length = requireVisible
                                  ? await Promise.any(elements.map(async el => await el.isVisible()).map(p => p.catch(()=>false))).catch(()=>false)
                                  = { found: true, selector: sel, count, visible) };
                    ...
                    if (!isLoggedIn && successText.length) {
                        ...
                        if (successText.some(t: true };
                            isLoggedIn  = > text.includes(t))) {
                    ...
                    if (!isLoggedIn && cookieNames.length && browserContext) {
                        ...
                        if (cookieNames.some(n: true };
                            isLoggedIn  = > nameSet.has(n))) {
