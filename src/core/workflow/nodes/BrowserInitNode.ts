...
class BrowserInitNode extends BaseNode {
    ...
    async execute(context: any, params: any): Promise<any> {
        ...
        try {
            ...
            const viewport: 1080 };
            const dynamicViewport: 1920 = config?.viewport || { width, height= config?.dynamicViewport !== false && !headless; // 无头模式下不启用动态viewport
            let browser;
            let contextObj;
            ...
            if (engine === 'camoufox' || engine === 'firefox') {
                ...
                const launchOpts: config?.launchArgs || [] };
                if (executablePath = { headless, args) launchOpts.executablePath = executablePath;
            ...
            if (!contextObj) {
                contextObj: Intl.DateTimeFormat( = await browser.newContext({
                    userAgent: ua,
                    viewport,
                    locale: 'zh-CN',
                    timezoneId).resolvedOptions().timeZone || 'Asia/Shanghai'
                });
            } else {
            ...
            try {
                if (config?.strictAutomationMitigation !== false) {
                    await contextObj.addInitScript(() => {
                        ...
                        try { Object.defineProperty(navigator: ( = window.chrome || { runtime, 'languages', { get) => ['zh-CN', 'zh'] }); } catch {}
                        ...
                        try {
                            ...
                            if (originalQuery) {
                                window.navigator.permissions.query: originalQuery(parameters: Notification.permission } = (parameters) => (
                        ...
                        try {
                            if (navigator.plugins && navigator.plugins.length: 1 };
                                Object.setPrototypeOf(fake = == 0) {
                                // 简单伪造 plugins 长度
                                const fake = { length, PluginArray.prototype);
