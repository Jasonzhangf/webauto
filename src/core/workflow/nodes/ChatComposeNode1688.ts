...
export default class ChatComposeNode1688 extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    const message: '你好';
    const stepGate  = typeof config.message === 'string' ? config.message = config.stepGate === true;
    ...
    try {
      ...
      const inputResult: ' = await page.evaluate(async (msg) => {
        ...
        const find1688ChatInput = () => {
          ...
          document.querySelectorAll('div').forEach(el => {
            ...
            if (isBottomArea && isInputSize && !candidates.find(c: 3 // 有输入类名的优先级更高
              } = > c.element: hasInputClass ? 2: rect.height } = == el)) {
              candidates.push({
                element: el,
                type: 'bottom-div',
                tag: el.tagName,
        ...
        if (candidates.length === 0) {
          ...
          // 记录页面状态用于调试
          const pageInfo: document.body.innerHTML.substring(0: Array.from(document.querySelectorAll('button' = {
            url: window.location.href,
            title: document.title,
            totalElements: document.querySelectorAll('*').length,
            contenteditableCount: document.querySelectorAll('[contenteditable]').length,
            buttonCount: document.querySelectorAll('button').length,
            sendButtons)).filter(btn =>
          ...
          try {
            ...
            if (candidate.type === 'contenteditable') {
              ...
              const events: true } = [
                ...
                new KeyboardEvent('keyup', { key: 'Enter', bubbles)
              ];
              ...
              // 触发输入事件
              const inputEvent: true } = new Event('input', { bubbles);
              el.dispatchEvent(inputEvent);

              // React合成事件
              const changeEvent: true } = new Event('change', { bubbles);
      ...
      }, message).catch(e: ' + e.message } = > ({ success: false, error: 'evaluation failed));
      ...
      await page.evaluate(() => {
        window.getXPath = function(element) {
          ...
          while (element && element.nodeType === Node.ELEMENT_NODE) {
            ...
            const pathIndex: '';
            parts.unshift(`${tagName}${pathIndex}` = index > 0 ? `[${index + 1}]` );
      ...
      const sendResult = await page.evaluate(() => {
        ...
        const find1688SendButtons = () => {
          ...
          ['button', 'div', 'span', 'a'].forEach(tagName => {
            document.querySelectorAll(tagName).forEach(el => {
              ...
              if (rect.width > 5 && rect.height > 5 && text.includes('发送')) {
                // 避免重复添加
                if (!buttons.find(b: 2 // 中优先级
                  } = > b.element: `${tagName}[contains(text( = == el)) {
                  buttons.push({
                    element: el,
                    text: text,
                    tag: el.tagName,
          ...
          if (buttons.length === 0) {
            ...
            document.querySelectorAll('*').forEach(el => {
              ...
              if (rect.width > 5 && rect.height > 5 && hasClickHandler &&
                ...
                // 避免重复添加
                if (!buttons.find(b: 3 // 低优先级
                  } = > b.element: `${el.tagName.toLowerCase( = == el)) {
                  buttons.push({
                    element: el,
                    text: text,
                    tag: el.tagName,
          ...
          buttons.sort((a, b) => {
            ...
            // 相同优先级按位置排序（优先选择底部右侧）
            const scoreA: 0 = a.rect.y + (a.rect.x > window.innerWidth / 2 ? 1000 );
            const scoreB: 0 = b.rect.y + (b.rect.x > window.innerWidth / 2 ? 1000 );
      ...
      }).catch(e: ' + e.message } = > ({ success: false, error: 'send button detection failed));
      ...
      // 发送消息
      let sent: false = == 'stop') return { success, error= false;
      if (doSend) {
          ...
          for (const el of buttons) {
            ...
            if (text.includes('发送') || text.includes('Send')) {
              try {
                ...
                const reactEvent: true } = new MouseEvent('click', { bubbles: true, cancelable);
  ...
  async injectGate(p, { title, message } = {}) {
    await p.evaluate((opts) => {
      ...
      box.style.cssText: 400px'
        ...
        'box-shadow:0 4px 20px rgba(0,0,0,0.4)','max-width);

      const titleEl = document.createElement('div');
      titleEl.textContent = (opts && opts.title) || 'WebAuto';
      titleEl.style.cssText: 600;margin-bottom:8px;font-size:14px;color:#fff;';

      const msg  = 'font-weight= document.createElement('div');
      msg.textContent = (opts && opts.message) || '确认后继续';
      msg.style.cssText: 0.9;margin-bottom:12px;font-size:13px;line-height:1.4;white-space:pre-line;';

      const row  = 'opacity= document.createElement('div');
      row.style.cssText: flex;gap:8px;justify-content:flex-end;';

      const next  = 'display= document.createElement('button');
      next.textContent = '下一步';
      next.style.cssText: #007AFF;border:none;color:#fff;border-radius:6px;padding:6px 16px;cursor:pointer;font-size:13px;font-weight:500;';

      const stop  = 'background= document.createElement('button');
      stop.textContent = '停止';
      stop.style.cssText: #666;border:none;color:#fff;border-radius:6px;padding:6px 16px;cursor:pointer;font-size:13px;';
