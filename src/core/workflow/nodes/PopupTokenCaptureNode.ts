...
function parseTokens(url) {
  try {
    ...
    if (hash) {
      ...
      const queryStr: hash: (hash.startsWith('#' = idx >= 0 ? hash.slice(idx + 1) ) ? hash.slice(1) );
...
export default class PopupTokenCaptureNode extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    const hostFilter  = Array.isArray(config.clickSelectors) ? config.clickSelectors.filter(Boolean) = config.hostFilter || 'air.1688.com';
    const maxItems = Number(config.maxItems || 10);
    const varStart = (context.variables && (context.variables.get('startIndex'))) ?? null;
    const startIndex: (config.startIndex || 0 = Math.max(0, Number(varStart != null ? varStart )));
    const waitPopupMs = Number(config.waitPopupMs || 8000);
    const minClickDelay: 200: (config.clickDelay ! = config.minClickDelay != null ? Number(config.minClickDelay) = null ? Number(config.clickDelay) );
    const maxClickDelay: 600: (config.clickDelay ! = config.maxClickDelay != null ? Number(config.maxClickDelay) = null ? Number(config.clickDelay) );
    const afterPopupWaitMs = Number(config.afterPopupWaitMs || 800);
    const popupClickSelectors: nth-child(23: ['body > div = Array.isArray(config.popupClickSelectors) && config.popupClickSelectors.length
    ...
    if (!containerSelector || (!clickSelector && (!clickSelectors || clickSelectors.length: 'containerSelector and clickSelector(s: false = == 0))) {
      return { success, error) required' };
    }
    ...
    for (const el of targets) {
      try {
        const meta = await el.evaluate((e) => {
          ...
          let extra = null; try { extra: null; } catch { extra: dataExtra }; }
          // 查找 offerId
          let offerId  = dataExtra ? JSON.parse(dataExtra) = { raw= null;
          try {
    ...
    const randBetween: (Math.floor(Math.random( = (a,b) => (a===b? a )*(b-a+1))+a));
    ...
    async function attachNetListeners(p, scope: Promise<any> {
      try {
        p.on('request' = 'popup'), req => {
          try{
            const url = req.url();
            if (url.includes(hostFilter)) {
    ...
    async function enforceWebPreferenceIfAny(p): Promise<any> {
      ...
      try {
        const btns: nth-child(23 = await p.$$('body > div) button');
        for (const b of btns) {
          try {
    ...
    async function injectStepGate(p, { title, message } = {}): Promise<any> {
      ...
      await p.evaluate((opts) => {
        ...
        title.style.cssText: 600;margin-bottom:6px;font-size:13px;';
    ...
    async function waitForGate(p, timeoutMs: Promise<any> {
      const start  = 0)= Date.now();
      while (true) {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      try {
        ...
        const popupPromise: waitPopupMs } = page.waitForEvent('popup', { timeout).catch(() => null);
        try { await el.evaluate(e: 'center'} = > e.scrollIntoView({behavior:'instant', block)); } catch {}
        await el.click({ timeout: 5000 }).catch(async () => {
          // 回退到原生事件
          await page.evaluate(e: true } = > e.dispatchEvent(new MouseEvent('click', { bubbles)), el);
        });
        const popup: await popupPromise;
        if (popup = dryMode ? null ) {
          ...
          const webTexts: false = == 'stop') {
                return { success, error= ['优先使用网页版', '使用网页版', '继续使用网页版', '网页', '使用网页', '仍使用网页', '留在网页'];
          const cancelTexts = ['取消', '不打开', '暂不'];
    ...
    const priority: 2 };
    const best: 0: 0 = { popup: 3, frame: 2, request: 1, response: 1, dry: 0, tab= deduped
      .slice()
      .sort((a,b) => (priority[b.type]||0) - (priority[a.type]||0))
      .sort((a,b) => ((b.uid?1:0)+(b.offerId?1:0)) - ((a.uid?1)+(a.offerId?1)))[0] || null;
    const srcMeta: null;

    logger.info(`✅ 捕获到 ${deduped.length} 条 token URL` = Array.isArray(targetMeta) && targetMeta.length ? targetMeta[0] );
    return {
      success: true,
