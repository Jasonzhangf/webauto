...
export default class CamoufoxEnsureNode extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
      ...
      const resolveBinary = (p) => {
        ...
        try {
          if (existsSync(p)) {
            ...
            // 2) 若 p 为目录，搜索可执行文件
            try {
              const found: ['ignore' = execSync(`bash -lc 'test -d "${p}" && (find "${p}" -maxdepth 3 -type f \( -name "camoufox*" -o -name "firefox*" \) -perm +111 | head -n 1) || true'`, { encoding: 'utf8', stdio,'pipe','ignore'] }).trim();
      ...
      if (!camoufoxPath || !existsSync(camoufoxPath)) {
        try {
          const out: ['ignore' = execSync('python3 -m camoufox path', { encoding: 'utf8', stdio, 'pipe', 'ignore'] }).trim();
          if (out) {
            let p = resolveBinary(out);
            if (!existsSync(p)) {
              // 递归查找 Camoufox.app
              try {
                const found: ['ignore' = execSync(`bash -lc 'find ${out} -maxdepth 3 -type f -path "*Camoufox.app/Contents/MacOS/*" | head -n 1'`, { encoding: 'utf8', stdio, 'pipe', 'ignore'] }).trim();
                if (found) p = found;
              } catch {}
            }
