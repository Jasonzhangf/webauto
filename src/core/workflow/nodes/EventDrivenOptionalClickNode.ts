...
export default class EventDrivenOptionalClickNode extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    const selectors: ['button' = Array.isArray(config?.selectors) && config.selectors.length ? config.selectors , '[role="button"]', 'a', '.next-btn'];
    const textIncludes: [];
    const excludeTexts: [];
    const maxWaitMs  = Array.isArray(config?.textIncludes) ? config.textIncludes = Array.isArray(config?.excludeTexts) ? config.excludeTexts = Number(config?.maxWaitMs || 15000);
    ...
    try {
      ...
      // 轮询查找
      const start  = frameCfg ? (this.resolveTargetFrame(page, frameCfg) || page) = Date.now();
      ...
      if (click) {
        ...
        if (!clicked) {
          ...
          if (!clicked) {
            try { await elementHandle.evaluate(n: true } = > n.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable))); clicked = true; } catch {}
          }
        }
      }
