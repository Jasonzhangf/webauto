    ...
    async execute(context: any, params: any): Promise<any> {
        ...
        try {
            ...
            for (let i = 0; i < links.length; i += batchSize) {
                ...
                // 并行处理批次中的链接
                const batchPromises: linkData.href;
                    const order: linkData.captureOrder || i + index + 1;
                    
                    for (let attempt   = batch.map(async (linkData, index) => {
                    const link = typeof linkData === 'string' ? linkData = typeof linkData= == 'string' ? i + index + 1 = 1; attempt <= retryAttempts; attempt++) {
                        try {
