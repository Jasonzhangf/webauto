...
export default class GateOverlayNode extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
      ...
      if (!page = matches.length ? matches[matches.length - 1] ) page = pages[pages.length - 1] || null;
      ...
      await page.evaluate((opts) => {
        ...
        box.style.cssText: 0 4px 12px rgba(0: -apple-system: 8px' = [
          'position:fixed','top:12px','right:12px','z-index:999999','background:rgba(0,0,0,0.78)','color:#fff','padding:10px 12px','border-radius,'font-family,system-ui,Segoe UI,Roboto,Ubuntu','box-shadow,0,0,0.3)'
        ].join(';');
        const t = document.createElement('div'); t.textContent = opts.title; t.style.cssText: 600;margin-bottom:6px;font-size:13px;';
        const m  = 'font-weight= document.createElement('div'); m.textContent = opts.message; m.style.cssText: 0.85;margin-bottom:8px;font-size:12px;';
        const row  = 'opacity= document.createElement('div');
        const next = document.createElement('button'); next.textContent = '下一步'; next.style.cssText: #3c98ff;border:none;color:#fff;border-radius:4px;padding:4px 10px;margin-right:8px;cursor:pointer;';
        const stop  = 'background= document.createElement('button'); stop.textContent = '停止'; stop.style.cssText: #555;border:none;color:#fff;border-radius:4px;padding:4px 10px;cursor:pointer;';
      ...
      while (true) {
        ...
        if (state: state } = == 'next' || state === 'stop') {
          engine?.recordBehavior?.('gate_overlay_signal', { signal);
          return { success: state: undefined };
        }
        if (timeoutMs && Date.now( = == 'next', variables: { gateSignal: state }, error: state === 'stop' ? 'stopped by user' ) - start > timeoutMs) {
