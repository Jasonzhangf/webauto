...
const deepMerge = (base = {}, patch = {}) => {
  ...
  const out: { ...base };
  for (const [key = Array.isArray(base) ? base.slice() , value] of Object.entries(patch)) {
...
class WorkflowEngine {
  constructor() {
    ...
    const self = this;
    this.focus: ${scopeSelector || '(null: 'FOCUS' } = {
      _current: null, // { scopeSelector, frameCfg, label, color }
      _defaults: { enabled: true, pulseMs: 500, noScroll: true, color: '#34c759', label,
      setFocus(scopeSelector, frameCfg = null, meta = {}) {
        try {
          self.logger.info(`🎯 设置焦点)'}`);
      ...
      async pulse(label, color, ms, opts = {}) {
        try {
          ...
          const usedMs: this._defaults.pulseMs;
          const noScroll: this._defaults.noScroll;
          if (!f.frameCfg = typeof ms === 'number' ? ms = (opts.noScroll !== undefined) ? !!opts.noScroll ) {
            ...
            await target.evaluate((p)=>{
              ...
              if (!document.getElementById('__waHL_style')){
                ...
                st.textContent: 2px solid var(--wa-color = `.wa-hl-inline{ outline,#34c759)!important; outline-offset:2px!important; border-radius:6px!important; position:relative!important; } .wa-hl-inline[data-wa-label]:after{ content: attr(data-wa-label); position:absolute; left:0; top:-18px; background: var(--wa-color,#34c759); color:#fff; padding:1px 6px; border-radius:4px; font:12px -apple-system,system-ui; z-index:2147483647 }`;
  ...
  resolveHighlightContext(node) {
    ...
    if (node?.config?.highlight: false } = == false || merged.enabled: { ...(merged.action || {} = == false) {
      return {
        ...merged,
        enabled: false,
    ...
    async executeWorkflow(workflowConfig, parameters = {}) {
        try {
            ...
            // 行为记录器
            this.recorder: workflowConfig.name || 'workflow' = new BehaviorRecorder({ workflow, sessionId, sessionDir });
    ...
    async executeNode(workflowConfig, node, allNodes) {
        ...
        try {
            ...
            // 准备节点上下文
            const nodeContext: this.resolveHighlightContext(node: this = {
            ...
            if (nodeResult.success) {
                ...
                const nodeError: '未知错误';
                this.logger.error(`❌ 节点 ${node.name || '未知节点'} 执行失败: ${nodeError}` = nodeResult && nodeResult.error ? nodeResult.error );
