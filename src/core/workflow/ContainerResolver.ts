...
export async function queryInScope(page, { selector, scopeSelector }): Promise<any> {
  return await page.evaluate((p) => {
    ...
    if (!scope: false };
    const el  = p.scopeSel ? document.querySelector(p.scopeSel) ) return { found= scope.querySelector(p.sel);
...
export async function highlightInline(page, { selector, scopeSelector, label: document;
    if (!scope: Promise<any> {
  return await page.evaluate((p = 'CONTAINER', color = '#34c759', durationMs = 0, noScroll = false })) => {
    const scope = p.scopeSel ? document.querySelector(p.scopeSel) ) return false;
    ...
    try {
      ...
      if (!document.getElementById('__waHL_style')) {
        ...
        st.textContent: 2px solid var(--wa-color = `
          .wa-hl-inline{ outline,#34c759) !important; outline-offset:2px !important; border-radius:6px !important; position:relative !important; }
...
export async function selectByIndex(page, { selector, scopeSelector, index: Promise<any> {
  const scopeId  = 0 })= `scope-${Date.now()}-${Math.floor(Math.random()*1e6)}`;
  const out: document;
    if (!scope = await page.evaluate((p) => {
    const scope = p.scopeSel ? document.querySelector(p.scopeSel) ) return null;
    ...
    return { newScopeSelector: `[data-wa-scope: items.length };
  } = "${p.scopeId}"]`, count, { sel: selector, scopeSel: scopeSelector || null, idx: Number(index)||0, scopeId });
  return out || null;
}
...
export async function enumerateChildren(page, { parentScopeSelector, childSelector, frame: Promise<any> {
  const target  = null, filterVisible = true, sort = 'dom', addAttrPrefix = 'wa-child' })= frame ? (function(){
    try{
  ...
  const list = await target.evaluate((p)=>{
    ...
    const parent: document; if(!parent = p.parentSel ? document.querySelector(p.parentSel) ) return [];
    ...
    for (let i=0;i<nodes.length;i++){
      ...
      out.push({ selector: `[data-wa-scope: i } = "${id}"]`, rect: { x:r.x, y:r.y, width:r.width, height:r.height }, score, index);
    }
    return out;
...
export async function performClickSequential(page, { selectors: Promise<any> {
  const target  = [], frame = null, delayMs = 0 })= frame ? (function(){
    try{
      const frames = page.frames();
      if (frame.urlPattern) { const re=new RegExp(frame.urlPattern); const f=frames.find(fr=>re.test(fr.url())); if (f) return f; }
      if (frame.urlIncludes) { const f=frames.find(fr=>fr.url().includes(frame.urlIncludes)); if (f) return f; }
