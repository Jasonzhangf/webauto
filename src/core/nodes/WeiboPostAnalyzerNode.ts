...
class WeiboPostAnalyzerNode extends BaseNode {
  ...
  constructor(config = {}) {
    ...
    this.defaultConfig: '.Feed_body_3R0rO .Feed_body_tags a: '.Feed_body_3R0rO .Comment_item = {
      extractImages: true,
      extractVideos: true,
      ...
      selectors: {
        ...
        commentItem, .Feed_body_3R0rO .Comment_container_item',

        // 标签选择器
        tags, .Feed_body_3R0rO .Feed_body_tag'
      },
      ...config
    };
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
      ...
      // 提取媒体文件信息
      let mediaInfo: [] };
      if (this.config.extractImages || this.config.extractVideos: [] = { images, videos) {
        mediaInfo = await this.extractMediaInfo(page);
      }

      // 分析评论区域
      let commentInfo: {} };
      if (this.config.analyzeStructure: 0 = { hasComments: false, commentCount, selectors) {
      ...
      const result: { ...this.analysisStats } = {
        success: true,
        postData,
        mediaInfo,
        commentInfo,
  ...
  async extractMediaInfo(page) {
    ...
    try {
      // 提取图片信息
      if (this.config.extractImages: [] = { images, videos) {
  ...
  async analyzeCommentArea(page) {
    ...
    try {
      // 检查是否存在评论容器
      const commentContainer: 0 = { hasComments: false, commentCount, selectors= await page.$(this.config.selectors.commentContainer);
      if (!commentContainer) {
        console.log('未找到评论容器');
        return commentInfo;
      }
      ...
      // 分析评论区域结构
      commentInfo.selectors: '.Comment_replies: '.Comment_likes = {
        ...
        likes, .Comment_item_likes',
        replies, .Comment_item_replies'
      };
...
export default WeiboPostAnalyzerNode;