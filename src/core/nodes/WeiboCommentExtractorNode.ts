...
class WeiboCommentExtractorNode extends BaseNode {
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
      ...
      const result: { ...this.extractionStats }
      };

      console.log(`✅ 评论提取完成 - ${mainComments.length} 条主评论 = {
        success: true,
        comments: mainComments,
        replies,
        commentTree,
        commentMedia,
        extractionStats, ${replies.length} 条回复, ${commentMedia.length} 个媒体文件`);
  ...
  async extractSingleComment(page, commentElement, commentId) {
    try {
      ...
      const commentData = await page.evaluate((element, selectors, commentId) => {
        ...
        // 检查是否有媒体内容
        const mediaContainer  = hasReplies ?
          replyContainer.querySelectorAll(selectors.replyItem).length = element.querySelector(selectors.commentMedia);
