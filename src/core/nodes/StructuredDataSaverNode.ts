...
class StructuredDataSaverNode extends BaseNode {
  ...
  async validateInput(input: structuredData' = {
      startTime: null,
      endTime: null,
      filesSaved: 0,
      totalSize: 0,
  ...
  async execute(context: any, params: any): Promise<any> {
    ...
    try {
      ...
      const result: { ...this.saveStats }
      };

      console.log(`✅ 数据保存完成 - ${savedFiles.length} 个文件 (${this.formatFileSize(this.saveStats.totalSize: this.generateExportPaths(savedFiles = {
        success: true,
        savedFiles,
        exportPaths),
        saveStats)})`);
  ...
  prepareJsonData(structuredData) {
    const data: 'Weibo Post Capture System' = {
      version: '1.0',
  ...
  async generateSaveReport(structuredData, savedFiles) {
    try {
      const report: structuredData.media?.length || 0
        }
      };

      const reportFilename: structuredData.comments?.length || 0 = {
        ...
        data: {
          ...
          comments,
          media= `capture_report_${structuredData.post?.postId || 'unknown'}_${Date.now()}.json`;
...
export default StructuredDataSaverNode;