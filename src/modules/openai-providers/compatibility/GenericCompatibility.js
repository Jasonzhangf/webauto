...
class GenericCompatibility extends ICompatibility {
  ...
  mapRequest(openaiRequest) {
    ...
    if (requestMappings.defaults) {
      for (const [field, defaultValue] of Object.entries(requestMappings.defaults)) {
    ...
    if (requestMappings.direct) {
      for (const [openaiField, providerField] of Object.entries(requestMappings.direct)) {
    ...
    if (requestMappings.transform) {
      for (const [openaiField, transformConfig] of Object.entries(requestMappings.transform)) {
        if (openaiRequest.hasOwnProperty(openaiField)) {
          if (typeof transformConfig === 'function') {
            providerRequest[openaiField] = transformConfig(openaiRequest[openaiField]);
          } else if (transformConfig.transform) {
            providerRequest[openaiField] = transformConfig.transform(openaiRequest[openaiField]);
          }
        }
      }
    }
  ...
  mapResponse(providerResponse) {
    ...
    if (responseMappings.direct) {
      for (const [providerField, openaiField] of Object.entries(responseMappings.direct)) {
    ...
    if (responseMappings.transform) {
      for (const [providerField, transformConfig] of Object.entries(responseMappings.transform)) {
        if (providerResponse.hasOwnProperty(providerField)) {
          if (typeof transformConfig === 'function') {
            openaiResponse[providerField] = transformConfig(providerResponse[providerField]);
          } else if (transformConfig.transform) {
            openaiResponse[providerField] = transformConfig.transform(providerResponse[providerField]);
          }
        }
      }
    }
  ...
  validateRequest(request) {
    ...
    for (const [field, rules] of Object.entries(requestMappings.validation)) {
      ...
      if (rules.min !== undefined && value < rules.min) {
        console.warn(`[${this.config.provider.name} Compatibility] Field '${field}' value ${value} is below minimum ${rules.min}, using minimum`);
        request[field] = rules.min;
      }
      
      // 检查最大值
      if (rules.max !== undefined && value > rules.max) {
        console.warn(`[${this.config.provider.name} Compatibility] Field '${field}' value ${value} exceeds maximum ${rules.max}, using maximum`);
        request[field] = rules.max;
      }
      
      // 检查数组最大长度
      if (rules.maxItems !== undefined && Array.isArray(value) && value.length > rules.maxItems) {
        console.warn(`[${this.config.provider.name} Compatibility] Field '${field}' array length ${value.length} exceeds maximum ${rules.maxItems}, truncating`);
        request[field] = value.slice(0, rules.maxItems);
      }
      
      // 检查允许的值
      if (rules.allowedValues && !rules.allowedValues.includes(value)) {
        throw new Error(`Field '${field}' value '${value}' is not allowed. Allowed values: ${rules.allowedValues.join(', ')}`);
      }
    }
  }
