...
export function createUiStateService({ bus, logger }: any = {}) {
  ...
  if (bus?.subscribe) {
    unsubscribers.push(
      bus.subscribe('ui.state.request', (payload = {}) => {
        const requestId = (payload as any)?.requestId || null;
        publishSnapshot((payload as any)?.reason || 'request', { requestId });
      }),
    );
  }
