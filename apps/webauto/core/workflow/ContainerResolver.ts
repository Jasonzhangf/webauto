...
export async function queryInScope(page, { selector, scopeSelector }): Promise<any> {
...
export async function highlightInline(page, { selector, scopeSelector, label = 'CONTAINER', color = '#34c759', durationMs = 0, noScroll = false }): Promise<any> {
  return await page.evaluate((p) => {
    const scope = p.scopeSel ? document.querySelector(p.scopeSel) : document;
...
export async function selectByIndex(page, { selector, scopeSelector, index = 0 }): Promise<any> {
...
export async function enumerateChildren(page, { parentScopeSelector, childSelector, frame = null, filterVisible = true, sort = 'dom', addAttrPrefix = 'wa-child' }): Promise<any> {
...
export async function performClickSequential(page, { selectors = [], frame = null, delayMs = 0 }): Promise<any> {
  const target = frame ? (function(){
    try{
      const frames = page.frames();
      if (frame.urlPattern) { const re=new RegExp(frame.urlPattern); const f=frames.find(fr=>re.test(fr.url())); if (f) return f; }
...
export function getContainerDef(lib, siteKey, name) {
  try { return lib?.[siteKey]?.containers?.[name] || null; } catch { return null; }
}

export function resolveQueryList(def) {
  if (!def) return [];
  if (Array.isArray(def.queries) && def.queries.length) return def.queries.map(q => q.css).filter(Boolean);
  if (typeof def.selector === 'string') return [def.selector];
  return [];
}

export async function findFirstInScope(page, { scopeSelector = null, queries = [], where = null, tagAsScope = true }): Promise<any> {
  return await page.evaluate((p) => {
    function visible(n){ try{ const s=getComputedStyle(n); if(s.display==='none'||s.visibility==='hidden'||Number(s.opacity)===0) return false; const r=n.getBoundingClientRect(); return r.width>4&&r.height>4; }catch{return false} }
    function okWhere(el, where){ if(!where) return true; try{ if(where.visible && !visible(el)) return false; if(where.hrefContainsAny && where.hrefContainsAny.length){ const h=(el.getAttribute('href')||'')+''; let pass=false; for(const t of where.hrefContainsAny){ if(h.includes(t)){ pass=true; break; } } if(!pass) return false; } return true; }catch{return true} }
    const parent = p.scopeSel ? document.querySelector(p.scopeSel) : document; if(!parent) return null;
    for (const part of p.queries){ const el = parent.querySelector(part); if (el && okWhere(el, p.where)) { const id = `scope-${Date.now()}-${Math.floor(Math.random()*1e6)}`; if (p.tag){ try{ el.setAttribute('data-wa-scope', id); }catch{} } return { scopeSel: p.tag?`[data-wa-scope="${id}"]`:null, elemSel: null, matched: part }; } }
    return null;
  }, { scopeSel: scopeSelector||null, queries, where: where||null, tag: !!tagAsScope });
}

export function getEnumeration(def){ try{ return def?.enumeration || null; }catch{ return null } }
