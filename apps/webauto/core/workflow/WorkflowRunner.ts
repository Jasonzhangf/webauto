...
class WorkflowRunner {
  constructor() {
        ...
        // Detect preflows config path: prefer new location under libs/workflows
        const preflowsOld = join(process.cwd(), 'workflows', 'preflows', 'enabled.json');
        const preflowsNew = join(process.cwd(), 'libs', 'libraries', 'workflows', 'preflows', 'enabled.json');
    ...
    async runWorkflow(workflowPath, parameters = {}) {
        try {
            ...
            // 执行工作流（允许 publish 回调透传）
            const result = await this.engine.executeWorkflow(workflowConfig, { ...parameters, sessionId: workingSessionId }, { publish: parameters.publish });
