...
async function main() {
  ...
  if (argv.help || argv.h) {
    console.log([
      ...
      '  --stage <name>               阶段：full|links|content|like|reply|detail（默认 full）',
      '                              links: 搜索+逐条点开采链(xsec_token, 单账号不分片); content: 搜索+采链+内容; like: 搜索+采链+内容+点赞; reply: 搜索+采链+内容+回复; detail: 仅开关帖子详情（回归压测）',
