...
import { executeEvent, executeOperation } from './action-executor.mjs';
  ...
  try {
    await page.exposeFunction('webauto_dispatch', async (evt: any) => {
      ...
      if (evt?.type === 'picker:event' && evt?.data?.eventKey) {
        ...
        try {
          const res = await executeEvent(page, opts.engine, opts.site, evt.data.eventKey, selector);
      ...
      if (evt?.type === 'picker:operation' && evt?.data?.opKey) {
        const selector = evt?.data?.selector || '';
        try {
          const res = await executeOperation(page, opts.engine, opts.site, evt.data.opKey, selector);
