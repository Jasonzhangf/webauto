...
test('main tabs keep xiaohongshu home and debug consolidation', async () => {
  ...
  assert.match(src, /type TabId = 'xiaohongshu' \| 'preflight' \| 'debug' \| 'settings';/);
  ...
  assert.doesNotMatch(src, /renderLogs/);
  assert.doesNotMatch(src, /\{ id: 'logs', label: '日志', render: renderLogs \}/);
});
